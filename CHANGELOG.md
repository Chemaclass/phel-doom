...
- Level 2 (the demon room) had no exit door, dead-ending the run before level 3: its `showcase-layout` shipped as a plain chamber with only the spawn, so a player who cleared L2 had no way to advance. Added the unlocked exit `D` on the east wall at the spawn row (matching the L3 layout). A new test now asserts EVERY level exposes an exit door reachable from spawn (BFS over the floor), so a missing exit can't ship again.
