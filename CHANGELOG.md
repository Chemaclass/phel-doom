...
- Far enemy sprites (quarter-res LOD, d > ~6 units) now render with readable humanoid silhouettes instead of uniform blobs: three load-time passes were added to the m2 mip build in `enemy_sprite.phel`. (1) `min-opaque=1` on the second box-filter keeps any texel where at least one of its four source pixels is opaque, preserving thin features (head, horns, limbs) that the prior 2-of-4 rule erased at quarter resolution. (2) Luminance contrast-stretch (min-range 40) expands the double-averaged pixel range back toward 0-255, restoring the light/dark separation crushed by repeated averaging. (3) Silhouette edge darkening (frac 0.40) darkens every texel that borders transparency by 40%, outlining the figure against both sky and floor. All three passes are load-time only; per-frame render cost is unchanged. Near and mid enemies are unaffected (m1 and native LOD paths unchanged).
- Enemy sprite bodies now read as warm olive-brown instead of neutral gray: a warm-bias redirect step was added to `rgb->256-lifted` in `tools/bake-enemy-sprites.phel`. After the gamma=0.50 lift, any pixel whose tone-lifted result falls on the gray ramp AND whose source RGB has r-b >= 12 (warm-tinted origin) is redirected to the nearest xterm-256 cube code where r_index > b_index. The imp's dominant dark-brown shadow pixels (source lum 7-35, r-b +16 to +36) now map to olive cube codes 58 rgb(95,95,0) and 94 rgb(135,95,0) instead of gray ramp codes, raising warm cube coverage from ~31% to ~80% of opaque imp pixels. Neutral source pixels (pure grays, the cacodemon's blue body) are unaffected. Enemy sprites were also rebaked from the Freedoom WAD using the corrected pipeline - previous rounds had applied in-place text patches to the data file rather than true rebakes, leaving residual gray-ramp codes that no amount of offset arithmetic could correct.
- Pickup sprites no longer render as pale gray slabs: the sprite data in `enemy_sprites_data.phel` was regenerated by a true WAD rebake (not in-place text substitution) using `rgb->256-pickup`, which applies only the gamma=0.50 tone-lift with no gray-ramp-shift and no warm-bias. The armor vest, ammo box, and soulsphere now show correct olive-green, brass, and blue colors respectively. The in-place patch applied in the previous commit subtracted 3 from the gray-shift offset but the underlying codes were already wrong (the gamma lift quantized bright Freedoom item palette entries to gray-ramp codes via an intermediate in-place correction chain that diverged from a clean rebake).
...
- Directional damage feedback is now 8-way: a hit paints a precise red arc on the screen edge the attacker came from - a centred bar on the matching edge for the four cardinals, an L at the matching corner for the four diagonals - instead of the old 4-way band, so you read the exact incoming bearing while i-frames are up. Driven by a new `:hurt-dir` octant (`attacker-octant`, the 8-way twin of `attacker-side`) stamped at the single hit site; the 4-way `:hurt-side` stays for the blood-drip columns. Single-shot feedback, kept under reduced motion; once-per-frame overlay, so the 3D hot loop is unchanged.
- High-contrast HUD accessibility toggle (Settings page, default off): un-dims the dim/grey HUD elements - the compass non-facing letters, empty heart pips, and a healthy ammo-reserve count render bold white instead of SGR-dim (2) / grey (90), so the HUD reads on washed-out or low-quality terminals. Driven by a new `:high-contrast` setting read once into the per-frame stats; pure overlay branch, so the 3D hot loop is unchanged.
- Two new settings, navigable on the pause/options page: **Crosshair** style (`cross` `+` / `dot` `·` / `open` `○` / `off` - the hit-marker still flashes when off) and an always-on **Run timer** that appends the elapsed run time (`M:SS`) to the HUD strip. Both persist to the settings JSON. The `:enum` cycle logic was generalised so each enum field carries its own `:choices` vector (Difficulty + Crosshair share one generic cycler), making future enum settings data-only. Pure config + once-per-frame overlay reads, so the 3D hot loop is unchanged.
- End-screen run summary + letter rank: the death and victory screens now show **accuracy %** (connecting trigger pulls / total pulls), **secrets** found/total, and a colour-coded **rank** S/A/B/C/D (gold S down to dim D) alongside the existing kills/time/bests. The grade is `0.7*accuracy + 0.3*secrets-ratio` (a secret-less run is never penalised), pure and deterministic. New `:shots-fired` / `:shots-hit` counters are bumped in the combat step (one trigger pull = one shot; a pierce/cone pull that strikes several enemies still counts as one hit; BFG/rocket counts on a kill) and accumulate across levels with kills/time/secrets. Counters are combat-step increments and the screens paint once, so the 3D hot loop is unchanged. Grade + accuracy live in `core/format` (unit-tested).
- Crosshair hit-marker: a connecting shot now flashes a confirmation over the reticle for ~0.12s - a bold-red `✗` on a kill, a bold-yellow `×` on a wound - so hits read instantly instead of only via blood/sfx. Driven by a new `:hit-fx {:kill? :ttl}` world field stamped in the combat step (single-target, pierce/spread, and BFG-splash paths) and decayed each frame; the render layer reads it once in the crosshair overlay. It fires only on confirmed contact (no marker on a miss or dry-fire), and is kept under reduced motion since it is single-shot feedback, not a strobe. Pure combat-flag + once-per-frame overlay, so the 3D hot loop is unchanged.
- Reduced-motion accessibility toggle (Settings page, default off; `PHEL_DOOM_REDUCED_MOTION=1` to force on): a photosensitivity-safe mode that gates the game's strobing horror beats. When on, the lights-flicker scanlines are dropped; the low-health heartbeat edge, the berserk border, the pulsing HUD labels (low-ammo / behind-you) and the powerup banners hold a steady state instead of pulsing; the jump-scare face flash is suppressed; and the door-eye drops its SGR-5 terminal hardware blink for a static eye. Essential single-shot feedback (the directional hit-vignette and the kill flash) is kept. Driven by a new `:reduced-motion` setting (persisted in the settings JSON) read once into the per-frame stats, so it is a pure once-per-frame overlay branch with no effect on the 3D hot loop. The settings panel widened slightly to fit the new row.
- The 2D minimap is now a framed HUD panel: a box-drawing border with a cyan `MAP` title strip wraps the floor plan, and the panel is inset one row from the top edge so it reads as a deliberate element instead of loose glyphs butting the screen corner. It stays flush to the right edge (the right border replaces the old edge-touching glyph column). Purely an overlay change, painted once per frame; the per-cell 3D loop still skips the panel region (now a bounded rectangle rather than a half-plane, so the top-margin row and the area below the panel render the real scene). Render-ms is within noise at 80x24/120x30/180x40 and the hot-loop closure count is unchanged. Fog-of-war, all pickup/enemy/door/switch markers, `--full-map`, and the narrow-terminal auto-scale are unaffected.
- Distance fog now fades toward a near-neutral light-grey haze tint instead of pure black, with a filmic (ACES-ish) brightness curve: distant textured walls and the themed floor converge on the same grey haze (aerial perspective) so depth reads as "hazy/far" rather than just "dark", and mid-tones gain contrast so the scene reads lit rather than linearly dimmed. The tint is deliberately near-neutral - a saturated blue-grey read as an out-of-place blue band through far openings in the near-monochrome levels, and the 256-colour palette can't hold a "barely cool" mid-grey. The new behaviour is baked entirely at load (`fade-256-fog` lerps each code toward the haze in true RGB and re-quantizes to the nearest 256-colour code via `palette/nearest-256`, rebuilding `tex-fade-table` and the themed sky/floor gradient LUTs), so the per-cell hot path is unchanged (one nested `aget`) and render-ms is within noise at 80x24 / 120x30 / 180x40. `PHEL_DOOM_FLAT_FOG=1` restores the legacy fade-to-black. The grayscale sky gradient and the enemy/seam fades are unchanged.
