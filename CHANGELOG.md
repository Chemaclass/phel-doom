...
- Staircases read as ascending steps (issue #322): the raycaster now tracks the nearest TWO floor risers per ray instead of one, so a stair run draws as a stacked pair of treads (a lit cap, a shadowed riser face, and a dark nose edge per step) rather than a single step plus flat ground beyond. Each L7 / L6 run is exactly two steps between tiers, so the second riser captures the rest of the flight you can see. The cast stays allocation-equivalent (one extra accumulator in the same DDA loop, a fixed handful of extra arrays, no per-column churn) and the new work is dead on flat levels, so every flat frame - including the golden render fixture - is byte-for-byte identical. The per-tier tint was also toned down from the earlier saturated neon-blue to a subtler cooled / warmed stone, so the raised tiers read as tinted rock instead of solid colour blocks. Measured cost is contained to the multi-tier rooms (a few percent on the stair scene, zero elsewhere).
