...
- Upgraded phel-lang 0.46 -> dev-main (past 0.48): drop-in (tests green, golden frames byte-identical). Already at `withOptimizationLevel(2)` with tagged hot-loop locals (`^int` / `^float`), so it gains 0.48's compiler lowerings - tag-typed native ops, `(** x 2)` -> `x*x`, `reduce` over a tagged vector as native `foreach`, tail-position `foreach`/`doseq` without an IIFE closure - plus main's multi-arity fixed-arity dispatch (skips variadic `__invoke` packing, ~1.5-2x per multi-arity call, larger under JIT) and ~30% faster startup from the 0.47 OPcache re-exec warm child. Clears PHP 8.3+ `ReflectionType` deprecations. Source now uses the new eager `mapv` / `filterv` / `vec` instead of `apply vector` over lazy chains (interleaved A/B bench: perf-neutral locally without JIT; wins land on JIT-enabled player machines).
