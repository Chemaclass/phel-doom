...
- `riser-shade`: a PLAIN dark slate cell (`tier-riser-fill`, `48;2;32;36;52`). Every step face reads as a flat dark border distinct from both the walls and the textured grey floor, so a stair edge never blends into either. This is deliberately colour-flat: no distance/wall-index shading and no per-tier tint (the earlier #373 `riser-shade-level` approach sampled the wall shade table, which made risers read as walls); legibility of the step comes from the face / lip / surface contrast, not from a shade gradient. Riser faces are usually one or two rows tall.
...
**Step lip** is one brighter-slate row (`tier-lip-fill`, `48;2;92;100;128`, pack code -13) at the TOP of every rise-face - the lit leading edge of that step's tread. It keeps individual stairs countable when the treads between them collapse to sub-row at a low viewing angle (where stacked dark faces alone would merge into one dark mass). It stays inside the dark slate family: a highlight, not a white line, honouring the "keep it dark" tier palette.

**Tier-top floor** is the raised-tier ground: the treads between steps and the platform capping the staircase, visible between the far wall's bottom and the nearest riser's top. It paints as a PLAIN dark slate cell (`tier-top-fill`, `48;2;56;62;82`) stored per column in `tier-top-tex`, one flat colour uniform across the band. That flat fill - distinct from the busy textured grey floor, one notch lighter than the darker riser face, capped by the brighter lip - is what makes a raised tier and its stair edges read at a glance. Earlier drafts sampled the floor stone texture here (issue #384, via `tier-top-row-dist` + a per-column fog level) so a tier looked like textured ground continuous with the floor; that was reverted because a tier that matches the floor is exactly what made platforms and stairs hard to parse. The flat fill is also strictly cheaper: no per-column `tier-top-row-dist` inversion, floor-texel lookup, or fog `pow` - just an `aset` of a constant, so the per-column closure that texturing added is gone (built `main.php` closure count dropped 26 -> 25). `src/core/projection.phel`'s `tier-top-row-dist` remains (unit-tested, and the basis for the ceiling mirror `ceiling-top-row-dist`) but is no longer on the render path.
