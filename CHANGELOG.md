...
- Mouse look and click to shoot (issue #246): modern-FPS controls in the terminal. Move the mouse to turn (yaw) and look up/down (pitch), left-click to fire (hold to spray auto-fire weapons). Enabled via xterm SGR mouse reporting (`\e[?1003h` any-motion + `\e[?1006h`); the pure parser in `glue/controls` (`mouse-look`) turns the reports into a per-frame angular delta plus a fire intent, and `commands/play` folds them into the camera and the existing shooting path. Because terminals report absolute cell coordinates with no pointer lock, the look comes from the delta between consecutive reports and clamps to zero at the screen edges (no recentering) - a best-effort mouselook within the terminal's limits. A Mouse on/off toggle and a Sensitivity slider (default 50% = neutral 1.0x) are added to the settings page and persisted. All keyboard controls are unchanged and the mouse is purely additive; with Mouse off the terminal never captures the pointer.
- Per-cell floor heights (issue #232): cells can carry a floor height, and the raycaster + renderer draw a DOOM-style step (a shaded vertical riser plus a flat cap for the step top) where a ray climbs onto a raised floor. The foundation for steps, platforms and pits; levels opt in via an optional `:floor-heights` map (none ship yet, so the world stays flat). With every floor at 0 the frame is byte-for-byte identical to before. Stepping up onto the floor (Z physics, #233) and textured step caps (#236) land in their own changes below.
- Per-cell ceiling heights (issue #235): the matching ceiling axis to #232. Cells can carry a ceiling height, and the raycaster + renderer draw a hanging ceiling (a shaded vertical face plus a flat cap for the underside) where a cell's ceiling drops below the viewer's, for low tunnels; a height above the default just lifts the ceiling into a taller atrium. Levels opt in via an optional `:ceil-heights` map (none ship yet, so the world stays flat). With every ceiling at the default 1.0 the frame is byte-for-byte identical to before. Textured ceiling caps follow with the floor caps in a later issue.
- Z physics: step-up and fall (issue #233): the player now moves on the Z axis over the floor heights from #232. The eye height follows the floor underfoot, a small step (up to 0.4 units) is auto-climbed when you walk into it, a taller riser blocks the move like a wall, and stepping off a ledge or into a pit eases you down under a smooth gravity. On the still-flat shipping levels nothing changes: the floor is everywhere 0, so the view stays grounded and the frame is byte-for-byte identical to before.
- Enemy sprites anchor to their cell floor height (issue #234): a monster standing on a raised platform now rises to stand on it (and one in a pit sinks) instead of floating at the horizon. The sprite, its grounding shadow, its face glyph and the floating HP digit all shift together by the screen projection of the cell's floor height (greater the taller the floor, smaller the farther away). Flat ground keeps every cell at height 0, so the shift is zero and the frame is byte-for-byte identical to before.
...
- Stair / platform / pit showcase level (issue #237): Level 2 is now a hand-authored verticality showcase that turns the floor-height (#232), ceiling-height (#235) and Z-physics (#233) features ON for players. You climb a four-step staircase onto a raised platform that overlooks the room, and drop into a sunken pit and walk back out. Each step rises 0.3 (at or below the 0.4 auto-climb limit) so the whole flight is walkable, a low ceiling lintel hangs over the stair foot, and a monster standing on the platform rises to stand on it (#234). Level 2 keeps its demons + imp roster, chase speed and shotgun drop, so the difficulty curve is unchanged. Level 1 stays flat and the run is still 10 levels with the boss arena last.
- Verticality across more campaign levels: walkable up/down terrain now reaches Levels 3, 5 and 7, not just the Level 2 showcase. Each gets a DISTINCT hand-authored feature: L3 a descending trench you walk down into and climb back out the far side, L5 a raised central dais (a vantage in the cyberdemon arena) reached by a ramp, and L7 a staircase onto a north-wall ledge through a low-ceiling tunnel. Every climb is at or below the 0.4 auto-climb limit so each flight is walkable, and pits drop one step per tier so you fall in under gravity and walk back out. Each level keeps its catalog identity (name, enemy mix, chase speed, keycard lock, weapon drop) and the exit door + keycard stay reachable on flat ground around every feature, so the difficulty curve and progression are unchanged. Level 1 stays flat (the golden-render fixture) and the run is still 10 levels with the boss arena last.
...
- Calmer 3D view: dropped the decorative blinks and strobes from the gameplay view. The cues that carried information are held steady instead (info kept, flashing gone): the low-health heartbeat edge and the berserk border no longer throb (steady tints), the low-ammo / behind-you / reload-reminder HUD labels and the berserk/invuln powerup banners stay visible the whole time their state is active (no on/off blink), the `JAMMED` chip and the low-health hearts strip are steady-bright, and doors and aggro-range enemy heads hold their bright frame instead of pulsing on a sin wave. Every terminal hardware-blink (SGR `\e[5`) usage is gone from the renderer. Essential single-shot feedback (directional hit-vignette, kill flash, dry-fire CLICK, crosshair hit-marker) and the gentle interactive-pickup glow are unchanged. The `Reduced motion` setting still persists but no longer needs to gate anything (the calm view is now the default for everyone). Event-gated, so no change to the steady-state frame: the golden render hashes are unchanged.
...
- Render hot-path: pin referred frame-math globals (`tex-fade-table`, `bg-cell-cache`, `seam-darken-*`) as frame-level locals in `frame->string`, eliminating per-row and per-cell `Phel::getDefinition()` calls; also hoist `floor-flat?` out of both per-row loops and `vh-1` out of the per-column loop in `compute-wall-shades`. Byte-identical output (golden hashes unchanged). Measured -5.9% at 180x40 on no-JIT local PHP (issue #262).

### Removed

- Removed three decorative horror effects that flashed without carrying information, for a calmer 3D view: the lights-flicker overlay (dark scanlines), the blinking door-eye (a hardware-blink red `ʘ` on doors), and the jump-scare face flash (a hardware-blink skull on enemies). Their dead state and physics drivers (`tick-flicker`, `tick-door-face`) were removed too.
