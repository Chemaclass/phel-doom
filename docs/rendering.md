...
**Feet anchoring.** The renderer stands each billboard on its feet, not centred on the horizon. The enemy zone pass computes the flat z=0 floor row (`round(svh/2 + 0.5*wall-px) + pr`) and sets the body to span from head to feet on that row. The grounding shadow drops on the foot row; the face glyph and floating HP digit ride the same anchor. `scene-pd` is halved in pixel-doubled mode, so the projection scales automatically. The golden hashes in `render-cache-test/test-frame-bytes-pinned` are unchanged because the golden fixture has no enemy on-screen.
