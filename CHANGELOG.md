...
- Mouse look and click to shoot (issues #246, #318, #320, #288, #324): a `+` crosshair fixed at screen centre with the gun firing dead-ahead; moving the mouse turns the camera (yaw) and looks up/down (pitch) proportional to the pointer motion, at a brisk default speed (a base `mouse-look-gain` over the raw column-for-column rate, since bare 1:1 reads slow for an FPS). A terminal cannot lock, warp or hide the OS pointer (iTerm2 included, #313), so pure delta would freeze the turn the instant the pointer hits a window border; a sustained edge-pan fallback keeps the camera turning past the edge instead. Left-click fires (hold to spray auto-fire weapons), via xterm SGR mouse reporting. A Mouse on/off toggle and a Sensitivity slider are added and persisted; with the mouse off the crosshair stays centre and play is byte-for-byte the keyboard-only build (golden fixture unchanged). On by default, keyboard controls unchanged.
