...
- View + weapon bob while walking (#411, #412): opt-in **View bob** setting (0-100%, off by default) nods the whole scene on a distance-driven walk cycle, and the first-person gun dips/sways on the same cycle. Cosmetic, never moves where a shot lands; byte-identical at rest.
- Room lighting (#418): opt-in **Room light** setting (off by default) folds a per-cell light bias into the wall shade so levels read as dark rooms with lit pools. Bias grid derived from the map at load (lamp pools, `core/light`), looked up one-per-column at the ray's hit cell (+0.5-2.5% render). Off is byte-identical; derived like `:pgrid` (rebuilt on grid change / load, never saved).
- Per-level floor theme (#417): each level's `:theme` tints its floor gradient (grey / steel / moss / clay / rust / hell) so episodes read distinct. Baked into the load-time LUT + keyed in the gradient memo, so it rebakes only on level change (zero per-frame cost). Walls + sky stay grayscale; `:base` is byte-identical to the old look.
...
- Upgraded symfony/console 7.4 -> 8.1 (and symfony/var-dumper for dev). phel-lang already declared `^6.0|^7.0|^8.0`, so only this project's own `^7.4` pin held it back; nothing in the CLI wiring needed changing. Verified through the built PHAR (`--version`, `play --help`, `demo --help`) plus a live render run, not just the test suite, since the suite covers game logic rather than console plumbing.
- Frame cap raised to ~120fps (was ~60fps): `standard-frame-us` 16667 -> 8333 (hardware renders 97-222fps uncapped). Input feel unchanged - per-frame constants converted to frame-rate-independent units: movement/turn/pitch/sprint hold counters count down in seconds, mouselook edge-pan (`mouse-yaw-rate-max` / `mouse-pitch-rate-max`) scaled by `dt`; resize-poll + mouse-re-arm cadences doubled (`resize-poll-frames` 6->12, `rearm-mouse-frames` 60->120). Render byte-identical. Savegames drop the `:moves` hold counters and load at rest, so old int-frame saves can't be misread as seconds of auto-walk.
- Mouse turn faster + survives the pointer leaving the window (#324, #313): `mouse-look-gain` 3.0 -> 5.0 so one swipe covers most of a turn. Edge-pan is now **ungated** - it keeps turning while the pointer sits in the outer border band, so a pointer pinned at/past a window edge keeps swinging (no terminal can lock/hide the OS pointer). A resting mid-screen pointer never drifts. Ghostty joins the startup cosmetic-arrow notice. Keyboard-only play unchanged.
- Upgraded phel-lang 0.46 -> 0.49: drop-in (tests green, golden frames byte-identical). Already at `withOptimizationLevel(2)` with tagged hot-loop locals (`^int` / `^float`), so it gains 0.48's compiler lowerings - tag-typed native ops, `(** x 2)` -> `x*x`, `reduce` over a tagged vector as native `foreach`, tail-position `foreach`/`doseq` without an IIFE closure - plus 0.49's multi-arity fixed-arity dispatch (skips variadic `__invoke` packing, ~1.5-2x per multi-arity call, larger under JIT), primitive return tags on core fns (`count` / `str` / `name` bindings now type-infer into the native-op emitter), native-comparison `if`/`and`/`or` tests spliced without the truthiness adapter, and ~30% faster startup from the 0.47 OPcache re-exec warm child. Clears PHP 8.3+ `ReflectionType` deprecations. Source now uses the new eager `mapv` / `filterv` / `vec` instead of `apply vector` over lazy chains (interleaved A/B bench: perf-neutral locally without JIT; wins land on JIT-enabled player machines).

### Performance

- Production builds strip symbol metadata (phel-lang 0.49 `withStripSymbolMeta`): docstrings, source locations and arity info are compile-time only, so `phel build` now drops them from the emitted PHP. Compiled game code 6.74 -> 5.18 MB of PHP text (-23%), phar 3.24 -> 3.04 MB, and cold start -7.2% through the phar / -10.9% through `out/main.php` (interleaved A/B, 15 rounds). Golden frame hashes are identical through the built artifact, not just the source suite. Trade-off: `phel doc` and `(meta ...)` over *built* defs return nil - the game never reads meta at runtime, and the repl / `phel test` / `phel run` compile from source, so dev workflow is untouched.
- Render memo/dedup pass: `frame-gradients` also caches the sub-row sky codes + four floor-cast tables (rebuild only on resize/pitch/px2 toggle); `emit-scene-px1` hoists `vw-1` + reads a precomputed per-column wall-height; `paint-face-overlay` / `paint-enemy-hp-flashes` reuse the zone pass's `enemy-projs`. Byte-identical; wins on resize/pitch/combat frames the fixed-viewport bench doesn't exercise.
...
- The start menu keeps its `ENTER / settings / quit` hints on short terminals. Sharing the end-screen box brought a row clamp with it, and the clamp trims from the bottom - so at 13 rows or fewer the key hints were the first thing dropped, leaving a menu with no visible way to start the game.
- A corrupt save no longer throws out of the play loop: the tagged-map decoder checks the payload on every level of recursion, not just the top, so a nested `["$m", <not-an-array>]` is rejected instead of reaching `array_keys`.
- A demo whose `keys` field is not a string is rejected at parse time instead of crashing on the first replayed frame, after the terminal is already in raw mode.
...
- The tech-talk demo session (`demo --phase N`) restores the terminal on a crash, matching the normal play session.
