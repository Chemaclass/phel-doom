...
The same mixer runs at both render scales: 2 sub-rows per cell at full detail, 4 per 2-terminal-row scene cell in pixel-doubled mode - the identical half-row absolute precision. At full detail it requires subpixel mode (`PHEL_DOOM_NO_SUBPIXEL=1` falls back to the whole-cell edge bands above). Costs ~15% render time at full detail (the per-cell mix-column checks), well inside the auto-calibration budget. The `Fast walls` setting (`:fast-walls` in stats; dev/bench override `PHEL_DOOM_FLAT_WALLTEX=1`) paints the wall INTERIOR as a single flat sample per cell while keeping this seam mixer running, so the silhouette stays sub-pixel and only the interior texture goes chunky - reclaiming ~6-14% render time and ~17% bytes/frame on large screens. Off by default (byte-identical to the shipped render); see [settings.md](settings.md).
