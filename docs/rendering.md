...
**Mip levels:** each baked sprite gets a load-time mip chain (`sprite-mips`: half and quarter resolution, box-filtered in RGB space and requantized to xterm-256; a 2x2 block stays transparent unless at least two sources are opaque, keeping silhouettes tight). The zone pass calls `sprite-for-type-lod` with the billboard's on-screen sub-row count (2 per cell): a source >= 4x taller than the box samples the quarter mip, >= 2x the half mip, else native. Mid and far monsters sample a pre-filtered image instead of point-skipping native texels, which kills most of the sparkle/speckle they used to show; near monsters keep the native level byte-identical.

**Distance fog:** the zone pass quantizes the glyph path's combined fade (`(dist/max-depth)²` capped 0.85, plus the wound tint below) to one of the 24 `sprite-fade` levels (same darkening curve as the textured-wall fog, with -1 mapped to itself so transparency survives the lookup) and stashes it per column in `e-fade`. Both emitters pass that level into `enemy-sprite-cell` / `enemy-sprite-quad`, which route every texel through the LUT - one extra aget per texel. A far monster now sinks into the same haze as the wall behind it instead of popping full-saturation, and a wounded one reads darker/dirtier just like the glyph path. `blit-sprite-into` applies the same depth-keyed fade to its billboards (corpses, blood splats, fireballs, near-LOD pickup sprites); close blits land on the identity level so pickup colours stay readable.
