...
- Full-detail frames render faster: 6% at 120x30, 12.5% at 180x45. The wall texture was the biggest cost in the frame, and most of it was each wall cell re-deriving what its column already knew. The emitter now resolves the wall band column by column, holding each column's texture constants once, and the row loop reads the finished cell. Frames are byte-identical across 1344 captures of every render mode.
- Runs on Phel 0.52. No game-code change: nothing in the codebase uses the `php/new`, `php/->`, `php/::` or `set-var` forms that 0.52 removes. Three interleaved pairs against 0.51 show no frame, cast or step change beyond noise.
- The step phase writes only what changed. Profiling `tick-world` on Phel 0.51 put a quiet frame at 448 us, most of it in map writes that changed nothing: one 20-key variadic `assoc` re-stamping seventeen timers already at 0.0, ten pickup rules rebuilding their vectors to find the player standing on nothing, every enemy re-writing its state, cooldown and hit-flash each frame, and repeated `weapon-spec` lookups. Timers now decay only while running, latches write only on change, pickups probe the cell before allocating, a trigger pull resolves its weapon spec, aim angle and wall ray once, and the per-frame world and enemy params carry the persistent-map tag so every keyword read compiles to a `find` call. Seven commits, each measured with `tools/bench-ab.sh` against its parent: `step-120` -29% and `step-fire-120` -32% over the branch, five interleaved pairs against main. Frames are byte-identical (render goldens unchanged).
...
- A kill plays the monster's own death cry on top of the generic kill cue. The cries shipped in #480, but the lookup searched the world from before the shot, when the monster was still alive, so no kill ever played one.
...
- A Doom-style message line names what just happened (#456): `Picked up a heart.`, `Picked up the BLUE keycard.`, `You got the SHOTGUN!`, `A secret is revealed!`, and the weapon plus its ammo on a slot switch. Every pickup used to be the same door tink and the item vanishing, so you could not tell an armor shard from an ammo box or see that a pickup had auto-switched your weapon. One steady row for 2 seconds, clipped to the viewport and hidden on very short ones; the frame is byte-identical when there is nothing to say.
- Enemies show their wind-up (#457): a steady `!` above the head during the 0.3-0.8s before a swing or a fireball. Dodging that window is meant to be the skill, but in sprite mode there was nothing to read - the billboard is one baked frame and the face overlay is skipped, so a far cacodemon about to fire looked exactly like a dormant one. Costs one pass over enemies the frame had already projected.
- Monsters have an attack pose (#463). One baked frame per type meant a monster mid wind-up looked exactly like one standing still, leaving the `!` telegraph to carry the whole read; now the sprite itself changes. All eight types, mipped like the resting poses. Costs 24 KB of phar and 28 ms of startup. The walk cycle is deliberately left out: a two-frame alternation at terminal resolution risks reading as silhouette flicker at mid range.
- The crosshair turns red when a shot would connect (#458). Since aiming became vertical (#243) a shot only lands when the crosshair is on the enemy's drawn sprite, so aiming a little at the floor or sky misses - and the miss read as lag, because nothing confirmed the target before the trigger. The cue asks the real selection rule for the weapon in hand (single-target hitscan, or the shotgun's cone, which disagree in both directions at shotgun range), so it cannot disagree with the gun about where a shot goes. It does not track the trigger: an empty mag or a running reload keeps the target red, since the dry CLICK and the reload prompt already say that. Kill, wound and muzzle-flash colours still take priority.
- The run has beats where it used to have cuts (#470). The killing frame is held for 0.6s before the YOU DIED box (input during it is dropped, so a panicked keypress cannot pick something on the screen that follows), and clearing a level shows a card with its kills, secrets and time - numbers the game already tracked but only ever showed once the whole run was over. Any key continues; both are skipped under `--demo` replay.
- Secret walls have a tell (#469). They looked and cast exactly like ordinary walls, so the only way to find one was to walk the level pressing F at every surface - while secrets are worth 30% of the run rank. A secret's stone now runs half a tile out of phase with the walls beside it, which is Doom's own misaligned-texture cue: visible when you look for it, invisible when you do not. The cast and the collision are unchanged, and a level without secrets renders byte-identically.
- The first level opens with a line of key hints (#467). One dim, steady line along the bottom - move, look, fire, use, help - for fifteen seconds on level 1 only, and it goes early once you have moved, turned and fired. The help panel exists but has to be discovered, and the objective splash says what to do, never how.
- The scroll wheel and `[` / `]` cycle weapons (#464). The wheel was parsed and thrown away and there was no next / prev key at all, so switching meant reaching for a number mid-fight. Steps through the weapons you own, wrapping both ways, with the same reload lockout and ammo bookkeeping as a slot key; an explicit number still wins in the same frame.
- The settings page explains itself (#468). Seventeen fields read as one undifferentiated list with nothing saying what any of them did. Fields are grouped under Audio / Video / Play / Controls / Access, with a one-line hint for the field under the cursor - both from the same table that drives the page, so a new field cannot ship without them. On a short terminal the list scrolls around the cursor instead of the box growing past the screen.
- New **HUD glyphs** setting (#471). The HUD uses a keycard, a half-heart, an info mark and a bullet; a font without those codepoints draws tofu boxes, which are frequently double width - they do not merely look wrong, they shift every column after them and break the strip's alignment. `ascii` swaps each for a single-column stand-in, and a non-UTF-8 locale picks it automatically on first run (a saved choice always wins).
- New **Texture filter** setting (#462, off by default): far walls sample a pre-filtered smaller copy of the stone texture instead of point-sampling the full 64x64, which is what makes distant walls sparkle as you move. Walls and floor both. On the L1 fixture the wall band's colour churn drops from 82.5% to 79.1% and the floor's horizon band from 81.7% to 80.5%, with the emitted frame 2.1% smaller and frame time unchanged either way. Off is byte-identical to before. The near floor is left alone - those rows magnify the texture rather than minify it, so a smaller copy would only blur them.

### Changed

- Quitting and restarting ask before they act, and the game pauses when you alt-tab away (#454). `q` sits one key from `w` and used to end a run on the spot; in game it now opens the pause menu with the cursor on Quit, so the confirmation is the second `q` (or Enter on that row). From the pause page `q` still quits and still persists settings; the start menu and end screens are unchanged. Restart asks the same way: the first select arms it (`Restart?  enter again`), moving the cursor disarms it. The terminal now reports focus (`\e[?1004h`), so losing the window pauses a live run and drops the held movement instead of leaving you standing in front of an enemy. Inside tmux the pane needs `focus-events on`.
- The damage flash is dark red instead of full-screen white (#465). Doom's pain language is red; the white wash was the last full-screen strobe in the calm 3D view, and at the rate an enemy can land hits it blanked the attacker for two frames at exactly the moment you needed to see where the damage came from. Same 0.05s duration.
- The attack-windup `!` is warning amber for every monster, matching the low-ammo and reload prompts. It used to take the enemy's own head colour, which comes from the glyph era and has nothing to do with the sprite drawn today - a cacodemon's catalog head is bright cyan while its sprite is red - so the mark read as information rather than a warning, and differently per monster.

### Fixed

- Dying no longer strips your arsenal (#453). Both death-screen retries (`r` fresh map, `R` same seed) restarted the level with the pistol and backpack 0, so an L7 retry met archviles with a starting weapon and no way back up: kill loot only refills weapons you already own, and a level seeded only its own debut weapon, so L8-L10 had nothing on the floor at all. A retry now re-enters with the rack you ENTERED that level carrying - weapons, backpack stack and the gun in hand - while kills, time, lives and ammo still reset. Entry rather than death rack on purpose: weapon-pickup cells are drawn from the run PRNG before the ammo boxes, so rebuilding with a weapon grabbed during the fatal attempt would shift every later spawn and break the identical replay `R` promises. Levels also seed the weapons you are owed (most recent debut first, two at most), so `--level=N` starts with a usable rack too.
- Movement stops when you let go, on terminals without key-release events (#461). Terminal.app, GNOME Terminal and xterm can only report that a key produced a byte, so every byte re-armed a 300ms hold - and since the OS auto-repeats while a key is down, the last repeat kept the player walking ~300ms after release: past the doorway, into melee range, through the strafe meant to dodge a bolt. The 300ms bridges the OS initial-repeat delay, which is only paid on the first byte; a byte arriving while the slot is still warm is a repeat and now gets 150ms. Kitty-protocol terminals were never affected.
- Enemies stop respawning in plain sight (#455). A revived enemy only had to be 3 units away - a few cells of open floor, well inside the view - so a monster could pop into existence mid-room while you watched. Respawn now also requires the cell to be out of your line of sight, keeping a far-enough cell as a fallback so an open arena still brings its dead back.
- The help and pause menus no longer paint past the bottom of the screen (#499, #500). The H info panel could drop its CONTROLS and COMPASS sections, but the rest was 28 rows - four more than an 80x24 terminal - and overflow does not clip politely: a terminal clamps every write past its last line onto that line, so the tail of the weapons table and the closing border landed on the last row on top of each other. Every block is droppable now, added in priority order (the RUN and PLAYER lines survive longest), and on a short terminal the blank spacer rows go too, fitting the whole panel into 24 rows with the weapons table intact. The pause menu does the same below 14 rows. What each height shows is monotonic - a shorter terminal never shows more than a taller one - and `centred-box-string` truncates as a last resort.
- The HUD strip no longer paints through the minimap (#466). With the map on at 80 columns, a run carrying a backpack, stamina, a key and a difficulty tag ran the row-2 strip straight through the panel's top border. Optional chips are appended only while they fit, dropping in reverse priority - run timer, difficulty, keys, backpack, stamina - so a squeeze costs decoration rather than the resource you spend mid-fight, and the core (level, kills, weapon, ammo) is never trimmed. The help menu also lists sprint and the mouse, and the weapon slots read `1 .. 7`: the docs, the menu and the README all still advertised eight, and combat.md documented a super shotgun that does not exist.
- The enemy face glyph sits on its enemy again in pixel-doubled mode (#477). The overlay projected with the full numerator while the pixel-doubled scene places bodies with half of it, so an enemy one world unit off centre had its face 12 terminal columns away from its body, and one three units off had it 34 columns away - far enough to land on a different enemy or off screen. The overlay also reads the depth buffers at that column, so a face could leak past a wall or vanish behind one that was not there. Only ever visible in glyph mode, which is why no golden frame caught it.

### Performance

- Level generation is 3x faster: 42.7 ms per level down to 13.9 ms, and up to 78 ms on the biggest level before. The two passes that dominate it - dropping the exit at a random reachable wall, and sealing any floor pocket the scattered wall blobs cut off - both flood the floor and scan every cell, and both read the grid through the persistent vector accessor while keying their seen-set by a freshly built `"x:y"` string per neighbour test. Reading through a flat PHP-native mirror, keying by that flat index, taking neighbour offsets from an int array, and writing the wall blobs into the mirror takes `place-exit` on a 64x40 grid from 57.9 ms to 8.4 ms and the pocket seal from 58.1 ms to 10.1 ms. Levels are unchanged: the flood consumes no rng and the scan order is the same, so all 210 worlds (10 levels x 7 seeds x 3 difficulties) hash identically.
- Firing is cheaper. Every shot floods the map from the player's cell so enemies in the same room hear it, and that flood cost 1.18 ms a call - for a radius of three cells, at most about 25 of them. The fill was never the problem: its seen-set was a set of coordinate vectors (a persistent-vector hash per membership test), its queue was rebuilt from scratch every step, and each cell allocated two more vectors. On native arrays it takes 93 us, bringing a frame in which you are shooting from 1.49 ms to 1.09 ms of state work before the renderer starts. Who wakes is unchanged, checked over 320 cases.
- Every frame is cheaper. The fog-of-war scan - which decides what the minimap has revealed - ran every frame, walking a line of sight out to every cell in range. What it computes depends on which CELL the player is in, not where they are inside it, so standing still or walking across one tile re-derived an identical answer. It now runs only when that changes: a walking frame goes from 1.11 ms to 0.97 ms of state work, a standing one from 1.05 ms to 0.92 ms. Opening a secret or throwing a switch next to you still lights up immediately, and the full revealed map after 400 frames of play is identical.
...
- `tools/bench-ab.sh` compares two refs by INTERLEAVING them, which is the only benchmark form that survives a laptop's thermal drift. Measured in one sitting, the same commit read 4.65 ms, 5.09 ms and 6.91 ms as the machine warmed up, so running the old ref and then the new one can report a 20% regression or a 10% win for code that did not change - both were "measured" that way before this existed. It alternates the refs pair by pair, averages the per-pair deltas and flags mixed signs as noise. Used on itself: the twenty PRs since v0.17.0, which added eight per-frame overlays, come to +0.5 to +0.7% frame time, two of three rows within noise.
- The pre-commit gate is down to 41s. On top of the formatting fix below, `composer ci` was running the whole test suite twice - once warm through `composer test` (12s) and once cold through `composer check-deprecations` (28s) - for 40 of its 50 seconds. The cold run is a superset, so the warm one is gone; a plain test failure inside the gate now prints the failing test names and copies the full log aside. `composer test` on its own is unchanged.
- The pre-commit gate went from 173s to 50s. `composer format-check` was 124s of it - 70% of the wait, in the step that only answers yes or no - because `phel format` is quadratic in the number of elements in a single collection literal (phel-lang#3218) and the baked asset files are one huge literal each: `enemy_sprites_data.phel` alone took 94.7s. `format` / `format-check` now skip the four generated data files, which are written by the bake tools and never by hand; `composer format-all` still covers everything. Contributor-facing only, no runtime change.
- Sound and resize handling stop forking on the game thread (#459). Each sound effect was a fork+exec of a shell - 3.30 ms per event on the reference machine - and the chaingun fires 20 of them a second inside an 8.3 ms frame budget, so a kill frame layering report + kill + wound could blow the frame on its own. None of it appeared in the F3 cast/render split, so it read as mystery jitter. One long-lived shell now takes the sounds over a pipe: 0.0012 ms per event. Resize detection moved from forking `stty size` every 12th frame (~5 ms, 10 times a second) to a SIGWINCH flag read at the frame checkpoint the loop already has, so the fork happens only when the terminal actually changed. Hosts without `proc_open` or ext-pcntl keep the old behaviour.
