...
**Feet anchoring**: A sprite is anchored by its feet on the flat z=0 floor row (`round(svh/2 + 0.5*wall-px) + pr`), not centred on the horizon. The renderer stands each billboard on that row and draws the body upward by its pixel height, so the body spans from head to feet and the feet land where the floor meets the wall at that distance. The grounding shadow, the face glyph, and the floating HP digit all derive from the same foot row. Combat's vertical hit gate uses the same anchor so a shot lands on the drawn body, not the floor below or sky above it.
