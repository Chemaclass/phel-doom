...
**Tier-top floor** is the thin band of raised-tier ground visible between the far wall's bottom and the nearer riser's top. It samples the floor stone texture (issue #384), so a raised tier reads as textured ground consistent with the main floor rather than a flat grey band. To keep the per-cell paint an aget (closure-tax rule), the sample is taken ONCE PER COLUMN inside `compute-riser-shades`, not per cell: `src/core/projection.phel`'s `tier-top-row-dist` (the height-aware row-distance inversion, `rowDist = (eye-z - z) * pd / ((row - horizon) * char-aspect)`, generalizing `build-floor-dperp`'s flat-floor special case to any tier height) recovers the perpendicular distance at the band's near edge (the riser-top row, using the tier's own `z1`); that distance drives one floor-texel + fog lookup (`tier-top-fog-level`, the `build-floor-level` gamma falloff evaluated for a single distance), and the resulting pre-painted BG cell is stored per column in `tier-top-tex`. The row loop's tier-top branch just agets it. The band is usually 1-3 rows tall (authored tier heights cap at fz 0.75), so a single per-column texel across those rows is a faithful approximation; a full per-row textured cast is still possible but unnecessary at this band size. This adds one per-column closure (the `^:pure` `tier-top-row-dist` inline) on tiered frames only - the per-cell hot loop stays closure-free.
