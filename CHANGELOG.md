...
- Weapon-fire extralight (issue #413, epic #375/#408): firing now briefly brightens the surrounding walls for a frame or two, so a shot reads as lighting the room, the feasible stand-in for a dynamic muzzle light on the PHP interpreter. It rides the existing near-death haze scalar as a brief negative shift and cannot overlap the red damage flash; not firing renders byte-identical to before.
- Weapon idle bob + sway while walking (issue #412, epic #375/#408): the first-person gun now dips on each footfall and sways side to side on the same walk cycle as View bob, so it reads as a held weapon instead of a static overlay. Reuses the View bob setting and `:bob-phase`: off by default and byte-identical at rest, raise **View bob** in Settings to enable both the scene nod and the weapon motion.
- View / head bob while walking (issue #411, epic #375/#408): a new opt-in **View bob** setting (0-100%, off by default) nods the whole scene (walls, floor, sky, enemies) on a distance-driven walk cycle, so moving reads as a person walking instead of gliding a camera. A `:bob-phase` on the world advances with the ground you cover and settles to 0 at rest, so a standing frame stays byte-identical; the bob is cosmetic and never moves where a shot lands (the hit gate keeps true camera pitch). Shipped look unchanged (default off); raise View bob in Settings to enable, lower it for motion sensitivity.
- Raised tiers and staircases render as 3D warm-stone geometry (epic #375; #370, #404): platforms and flights of stairs draw as countable steps with lit leading edges; faces and treads are textured as warm real stone (distinct from the neutral grey floor) and cast in true perspective; the floor you stand on takes your own tier's stone; and climbing raises the camera so the ground recedes and drops away when you look down from a height. Flat levels and level gazes stay byte-identical. Per-tier-height hue + off-tier floor cells tracked in #408.
- Enemies stand on and are occluded by variable-height geometry (epic #375; #394, #395): monsters anchor their feet to their own cell floor height, so they stand on platforms and treads (and stay correct when you are elevated); a sprite behind a raised platform or under a lowered ceiling is clipped by that nearer geometry instead of painting through it. Flat / flat-ceiling levels unchanged.
- Shots track enemy tier height (issue #396, epic #375): the vertical aim gate anchors on the enemy's own floor height, so an elevated monster is hit where it is drawn - at level aim it sits above the fixed crosshair, so aim up to connect. Flat-ceiling levels unchanged.
- Lowered ceilings and window gaps (epic #375; #385, #386, #387, #388): layouts author lowered ceilings via chars `a`/`b`/`c` (cz 0.75/0.5/0.25); a drop paints a hanging wall band, blocks walking unless the opening clears one `fz-step` (0.25), and stops shots at or below muzzle height while staying see-through above. Full-height levels unchanged.
- Per-cell floor + ceiling height data (epic #375; #368, #385): worlds carry `:fz-grid` / `:cz-grid` in quantized 0.25 steps; layouts author tiers via digits `1`/`2`/`3` (fz 0.25/0.5/0.75). Heights round-trip through saves with no version bump; shipped levels stay flat.
- Player + enemy step-up rule (#371, epic #375): the player and enemies climb a rise of at most one `fz-step` (0.25) and step down any depth; a bigger rise blocks like a wall. Flat levels: no-op.
- Staircase showcase level L2 (#372, epic #375): a 3-tread staircase climbs to a 0.75 platform with demons posted on it, spawning faced up the stairs so the verticality reads on entry.
...
- The floor you stand on recedes correctly from a raised tier (issue #416 part b, epic #375): standing on a platform, the near floor was cast at ground height z=0 with the raised eye, so it dropped away too fast. It now casts at the tier's own height, so a plateau recedes like normal ground at any elevation. Flat levels and ground-level play stay byte-identical. Descending stairs seen from above still render flat (stepped-descent geometry tracked in #416 part a / #408).
- Enemies route up stairs instead of stalling at a platform edge (issue #405, epic #375): the greedy chase (offsets up to ±135° at the player) stalled a monster against a tall multi-`fz-step` tier face. `advance` now builds one BFS flow-field per frame from the player over the one-`fz-step`-climbable graph and heads each enemy toward its next waypoint, routing around the ledge to the stairs and climbing. Built once and shared across enemies (tiered worlds only); flat worlds keep the greedy chase byte-identical.
