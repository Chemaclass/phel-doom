...
- Walkable verticality in the campaign (issues #237, #270): Level 2 is a hand-authored stair / platform / pit showcase, and Levels 3, 5 and 7 each gain a distinct feature - L3 a descending trench, L5 a raised dais in the cyberdemon arena, L7 a staircase onto a ledge through a low tunnel. Every climb is within the 0.4 auto-climb limit and the exit door + keycard stay reachable, so difficulty and progression are unchanged. Level 1 stays flat (the golden-render fixture); the run is still 10 levels with the boss arena last.
