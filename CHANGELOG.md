...
- Auto-calibrated pixel detail, always full screen. On a big terminal the interpreted-PHP render can't sustain a smooth framerate at full detail (the per-cell cost is ~linear in the cell count and dominates the loop; opcache/JIT give nothing measurable on it). The game renders at full detail for a few startup frames (hidden under the intro splash), measures the real render time, and - only when the machine needs it - locks a pixel-doubled mode: the scene renders at half resolution and each scene cell paints a 2x2 block, ~4x cheaper, while the view still fills the whole terminal. Framing, FOV and on-screen sprite sizes are identical to full detail (the cast keeps full-width FOV tables and every projection quantity scales in lockstep - no zoom when the mode kicks in), sprites keep their full 2x2 sub-pixel detail, and the sky/floor/walls keep FULL vertical sub-pixel colour fidelity (four vertical sub-samples per scene cell composed as half-block row pairs - the detailed look of the calibration frames does not wash out when the mode kicks in; only the horizontal axis gets chunkier). Stable for the session; recalibrates on a terminal resize. A faster machine keeps full detail.
- `--max-cols=N` / `--max-rows=N` flags override the auto mode: a positive value fixes an inset render size (smaller frame + blank border, anchored top-left), and `--max-cols=0` forces the full terminal at full detail (choppy on slow hardware). Unset = auto.

### Fixed

- Pixel-doubling now only ever engages on genuinely big screens (cell area beyond 200x45). A normal-sized terminal keeps full pixel detail even when the measured frame cost misses the smoothness budget - at that size detail beats framerate and the natural framerate stays acceptable. Big screens keep the existing behaviour (pixel-doubled only when the machine needs it).
...
- Floor texture cracks no longer read as harsh black speckle. The stone flat's hard dark crack pixels, sampled at the floor plane's 2x vertical grazing angle, scattered as noisy black dots down long corridors. The floor now samples a contrast-softened copy of the texture (each texel lerped toward mid-grey, codes compressed from 232-249 to ~237-246) so cracks calm to a gentle mottle; walls keep full contrast. Precomputed at load, so the floor path costs nothing extra.
- Crosshair background now matches the real cell under it. The reticle recomputed its backdrop with a simplified priority chain that used the flat wall shade, which mismatched the half-block textured wall actually drawn there and read as a fake light box. The centre cell rendered by the row loop is now captured and reused verbatim, so the `+` always sits on the true wall texture / floor / sky / sprite pixel.
