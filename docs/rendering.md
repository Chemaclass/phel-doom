...
**Tier floor tint (the floor you stand on, #404)**: when the player stands on a raised tier (`elevated?` = `eye-z > 0.5`), the near ground the camera casts IS that tier, so both emitters paint it the same warm `tier-riser-tex` stone the tread/riser faces use - via the tier-face ready-string path (pack `-2` in px2, a `cell-reg` string in px1), one representative floor texel per cell. So a platform reads as one continuous stone surface with its steps instead of the neutral grey base floor. Colour only: the floor GEOMETRY stays the eye-height z=0 cast above (a correct per-tier height projection is the issue's stated bonus, deferred), and because `tier-riser-tex` is fog-indexed (not per tier NUMBER), every raised tier shares one slate - distinguishing different tier HEIGHTS by hue, and tinting off-tier floor cells visible in the same column, are tracked in #408. `elevated?` is a frame constant, so ground-level play and every flat level are byte-identical (the new floor `cond` arm is a dead comparison there; the flat-floor dev override (`floor-flat?`, env `PHEL_DOOM_FLAT_FLOOR` only, not the in-game "Low detail" setting) takes one fixed `tier-riser-ramp` stone; hot-loop closures unchanged at 25).
