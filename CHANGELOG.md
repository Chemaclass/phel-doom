...
- The help and pause menus no longer paint past the bottom of the screen. The H info panel could drop its CONTROLS and COMPASS sections, but the rest of it was 28 rows - four more than an 80x24 terminal, the default nearly everywhere - and overflow does not clip politely: a terminal clamps every write past its last line onto that line, so the tail of the weapons table and the closing border all landed on the last row on top of each other. Every block is droppable now, added in priority order (the RUN and PLAYER lines, the reason you open it mid-fight, survive longest), and on a short terminal the blank spacer rows go too, which fits the whole panel into 24 rows with the weapons table intact. The pause menu does the same below 14 rows. `centred-box-string` truncates as a last resort, so no menu can paint off screen.
