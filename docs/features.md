...
- **Mouse look (#246, #324)**: a `+` aim crosshair follows the mouse 1:1 and the gun fires toward wherever it points (left-click to fire, held = auto-fire); pushing the reticle to a screen edge pans the camera so you can aim beyond the current view. On by default, toggled by the **Mouse** setting; edge-pan speed scales with **Sensitivity** (50% = neutral 1.0x). Purely additive - the keyboard path is untouched. See [input.md](input.md#mouse-look-issue-246).
