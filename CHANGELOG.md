...
- Upgraded phel-lang 0.46 -> 0.48: drop-in (tests green). Already at `withOptimizationLevel(2)` with tagged hot-loop locals (`^int` / `^float`), so it gains 0.48's compiler lowerings - tag-typed native ops, `(** x 2)` -> `x*x`, `reduce` over a tagged vector as native `foreach`, tail-position `foreach`/`doseq` without an IIFE closure - and clears PHP 8.3+ `ReflectionType` deprecations.
