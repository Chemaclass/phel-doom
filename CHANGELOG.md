...
- Frame cap raised to ~120fps (was ~60fps) with frame-rate-independent input timing: `core/perf.phel`'s `standard-frame-us` drops 16667 -> 8333 now that measured hardware renders 97-222fps uncapped. Every per-frame input feel constant was converted from a frame count to a seconds / per-second unit first so the higher cap doesn't change feel: the movement/turn/pitch/sprint hold counters (`glue/controls.phel`, `core/physics.phel`) now count down in seconds instead of decrementing by 1 per call, and the mouselook edge-pan rate (`mouse-yaw-rate-max` / `mouse-pitch-rate-max`) is now per-second and scaled by the frame's `dt`. The loop's resize-poll and mouse-re-arm cadences (`resize-poll-frames` 6->12, `rearm-mouse-frames` 60->120) double too, holding the same wall-clock cadence. Render output is unchanged (golden frame-byte hashes untouched). Savegames no longer persist the `:moves` input hold counters and always load them at rest - a mid-walk save loads standing still, and saves written by an older build (which stored int frame counts) can't be reinterpreted as seconds of uncontrolled auto-walk.
