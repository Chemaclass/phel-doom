...
- Mouse pointer hidden over the 3D view (issue #295): `mouse-enable` sends XTSMPOINTER `\e[>3p` to hide the OS pointer (not just the text caret) and `mouse-disable` restores it with `\e[>1p`; `render!` re-asserts `\e[>3p` every frame (gated on the Mouse setting, like the caret hide) so a mode flip / focus change can't resurface the arrow between rearms. Works on terminals that implement XTSMPOINTER (xterm-family); iTerm2, Apple Terminal and most others do not implement it, so the OS pointer stays visible there - no portable escape can hide it (the #313 limit), and with the fixed-centre mouselook its position is cosmetic (the turn reads pointer MOTION, not position). On those terminals the game prints a one-line startup notice (identified via `$TERM_PROGRAM`) that the centred `+` is the aim and the arrow is cosmetic. Gated on the Mouse setting; render frames byte-identical.
