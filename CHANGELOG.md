...
- L2 stands enemies on the staircase (epic #375 showcase): the demon room now pins fixed `:enemy-spawns` - two demons on the raised 0.75 platform and an imp on the 0.5 middle tread - so the enemy tier feet-anchoring (#395) and cross-tier sprite occlusion (#394) are visible in normal play, not only in tests. Entering from the spawn (looking north up the stairs) they read standing on the tiers with the lower imp partly hidden behind the nearer riser, then they chase down the treads at the player. Levels opt in via a per-level `:enemy-spawns` vector of `{:type :x :y [:lives]}`; `build-world` builds them via `new-enemy` with the same depth-scaled `:aggression` as the random roster and appends them (render reads each enemy's cell `fz` on the fly, no stored `:z`). Other levels are unchanged.
