...
```
aim-col =  reticle-axis(pointer_x, cols)                      ; 1:1, no sens
aim-row =  reticle-axis(pointer_y, rows)                      ; 1:1, no sens
...
- **`reticle-axis(p, len)`** maps a 1-based pointer cell `p` to the reticle's 0-based viewport cell: `clamp(p - 1, 0, len - 1)`. It is **always 1:1** (a 20-cell pointer move is a 20-cell reticle move) and returns an **integer** cell - the pointer report is a whole cell, so `p - 1` has no fraction to drift the painted `+` off the hitscan column or trip PHP's float-to-int deprecation in the render `intdiv`. Mouse **Sensitivity does NOT scale the reticle** (only the edge-pan rate below), so the `+` sits exactly where the mouse points at every setting - true 1:1 with the pointer and with the gun, which aims at the `+`. The **clamp** is the key: when the OS pointer sits at or past the window edge, the reticle **pins** at the first / last cell instead of being lost. Returns nil for a nil / `<= 1` axis.
...
The reticle math (`reticle-axis`, `edge-pan-rate`) stays **pure** in `glue/controls` and is unit-tested (1:1 tracking, edge clamp / pin, centre-region vs band, ease-in shape, sign, sensitivity scaling the pan only while the reticle stays 1:1, held-at-edge, return-to-centre stop, nil/degenerate axis). The escape re-assert that re-arms reporting lives in `io/` (next section).
