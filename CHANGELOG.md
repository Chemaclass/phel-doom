...
- Distinct riser shading + per-tier tint (#373, epic #375): `riser-shade-level` pulls riser faces darker than a coplanar wall and darker again by tier height (`z1` in fz-step units, 1/2/3), so a step never reads as a same-distance wall and taller rises stay distinct. All in `compute-riser-shades` (per column); flat levels byte-identical, tiered cost within noise.
- Textured tier-top floors (#384, epic #375): the raised-tier ground band now samples the floor stone texture instead of a flat grey gradient. Texel sampled once per column in `compute-riser-shades` via `tier-top-row-dist` (per-cell paint stays one `aget`); render cost within noise. Flat levels byte-identical; adds one per-column `^:pure` closure on tiered frames only.
