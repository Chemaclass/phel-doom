...
- New "Sub-pixel" compatibility setting (default on) for macOS Terminal.app (issue #332): the `▀` half-block cells that give the 3D view its vertical fidelity render with anti-aliased row seams / line gaps on Terminal.app (unlike iTerm2 / kitty / WezTerm / Ghostty, which draw them pixel-tight), breaking the half-block illusion into visible horizontal seams. Turning Sub-pixel off falls back to one solid colour per cell, which Terminal.app draws cleanly. Wires the existing `PHEL_DOOM_NO_SUBPIXEL` render path into a persisted player setting (render ANDs the two). Default on, so the shipped render is byte-identical (golden frame hashes unchanged). `docs/rendering.md` documents the root cause plus recommended Terminal.app settings (line spacing 1.0 also removes the seams with sub-pixel left on).
- New "Low detail" performance setting (default off): paints each wall AND floor cell as one flat colour instead of a two-sample `▀` half-block, trading interior vertical texture detail for ~11-26% less render time (the win scales up with screen size) plus ~31-40% fewer bytes per frame. Wall silhouettes / top-bottom edges keep their sub-pixel precision (the seam mixer still runs); sky and sprites are untouched. Off by default, so the shipped render is byte-identical (golden frame hashes unchanged); persisted with the other options and toggled on the settings page alongside Minimap / High contrast. Aimed at large terminals and slower hardware where the per-cell render loop dominates. The dev/bench override `PHEL_DOOM_FLAT_WALLTEX=1` forces it without touching saved settings.
- Living, non-repeating soundtrack: the OST no longer loops one fixed 3-second clip. `music/session-riff` concatenates several ornamented sections (`variant-riff` — the E-minor ostinato with random 3rd/5th/octave stabs, root +3/+7/+12, over the chugging root hits) into one long continuous track, seeded off the game pid so every session sounds fresh. It is baked once and looped whole, so there are no player-relaunch gaps between sections — only a brief one at the (much rarer) track wrap. Stays click-free (every note envelopes to zero, so section joins and the loop wrap never pop).
- Music pauses with the game: pausing (P menu, H info panel, or menu Resume) now SIGSTOPs the OST so it freezes in place and resumes exactly where it left off on unpause (`music/pause-music!` / `resume-music!`), instead of playing on under the pause screen.
- Random room layouts: every level now scatters random interior walls (`map/scatter-walls`), so the room disposition differs every run instead of being a flat empty box. Walls are sealed-connected - any floor a blob cuts off from the spawn is walled away - so pickups, enemies, the keycard and the exit are never stranded. The exit door can now land on ANY wall reachable from the spawn (an interior pillar or a border edge), not just the screen sides, so finding the way out is part of the game. The L10 boss arena keeps its hand-designed geometry (its switch target cells are placed, so it is left un-scattered). A test asserts every level (1-10) has interior walls plus exactly one reachable exit whose position varies with the seed.
- Mouse look and click to shoot (issues #246, #318, #320, #288, #324): a `+` crosshair fixed at screen centre with the gun firing dead-ahead; moving the mouse turns the camera (yaw) and looks up/down (pitch) proportional to the pointer motion, at a brisk default speed (a base `mouse-look-gain` over the raw column-for-column rate, since bare 1:1 reads slow for an FPS). A terminal cannot lock, warp or hide the OS pointer (iTerm2 included, #313), so pure delta would freeze the turn the instant the pointer hits a window border; an edge-pan fallback keeps the camera turning past the edge while you push the pointer toward the border (a pointer left resting never spins, so an unhideable OS arrow can't drift the view). Left-click fires (hold to spray auto-fire weapons), via xterm SGR mouse reporting. A Mouse on/off toggle and a Sensitivity slider are added and persisted; with the mouse off the crosshair stays centre and play is byte-for-byte the keyboard-only build (golden fixture unchanged). On by default, keyboard controls unchanged.
- Geometric mouse Sensitivity curve (issues #275, #306): the slider follows `3 ^ ((pct - 50) / 50)` - 50% = neutral 1.0x (saved settings unchanged), 0% drops to ~0.33x for fine aim, 100% reaches 3.0x for fast flicks - scaling the whole mouselook (the delta turn and the edge-pan rate) on top of the base `mouse-look-gain`.
- Mouse pointer hidden over the 3D view (issue #295): `mouse-enable` sends XTSMPOINTER `\e[>3p` to hide the OS pointer (not just the text caret) and `mouse-disable` restores it with `\e[>1p`; `render!` re-asserts `\e[>3p` every frame (gated on the Mouse setting, like the caret hide) so a mode flip / focus change can't resurface the arrow between rearms. Works on terminals that implement XTSMPOINTER (xterm-family); iTerm2, Apple Terminal and most others do not implement it, so the OS pointer stays visible there - no portable escape can hide it (the #313 limit), and with the fixed-centre mouselook its position is cosmetic (the turn reads pointer MOTION, not position). On those terminals the start menu (not a startup flash) carries the guidance: its how-to line notes the OS arrow is cosmetic and the centred `+` is the aim (terminal identified via `$TERM_PROGRAM`), and the controls list `mouse - look/fire`. Gated on the Mouse setting; render frames byte-identical.

### Changed

- Weapon fire volume now scales with the collision target's distance on a miss too: a shot that hits no enemy used to always play at full volume, but now it attenuates by the wall the shot collides with (`cast-wall-dist`) - a near wall reads loud, a far wall or an open level reads quiet, the same proximity feedback the enemy-hit path already gave. The report tells you how close the thing the shot hit is. Volume only (no gameplay effect); scaled by the global SFX setting as before.
- Calmer 3D view: dropped the decorative blinks and strobes from the gameplay view (every SGR `\e[5` hardware-blink is gone); informational cues are held steady instead (heartbeat edge, berserk border, HUD labels, powerup banners, the `JAMMED` chip, the hearts strip, doors, aggro-range enemy heads). Essential single-shot feedback (hit-vignette, kill flash, dry-fire CLICK, hit-marker) and the pickup glow are unchanged. Event-gated, so the golden render hashes are unchanged.
- Heart life-pickup plays the Hero cue (issue #281): the most valuable pickup shared the generic loot tink (`:door`) with armor, ammo and doors; it now uses the same `:berserk` (Hero) cue as the soulsphere. Reuses an existing allowlisted sound (no new asset); the render frame is unchanged.
...
- Sharper sidestep for dodging (game-feel pass): `physics/strafe-speed` 2.4 -> 3.0 u/s. Strafe stays slower than the 4.5 forward run (a tactical sidestep, not a teleport) but is now quick enough to clear a caster bolt's path during its wind-up, so dodging an archvile / baron bolt reads as a skill instead of a photo-finish.
- Kill-streak window widened 4s -> 7s (`combat/streak-window-seconds`): the streak chained the next kill only within 4s, but enemies respawn on a 3-6s band, so the counter reset to 1 on nearly every kill and the HUD streak sat dead at 1. Sizing the window above the respawn band lets a steady kill-per-respawn pace keep the streak alive. Scoring/HUD only, no gameplay effect.

### Performance

- Compiler call inlining (issue #166): `^:pure` lets opt level 2's `CallInliner` splice pure helpers at their call sites instead of dispatching through a PHP frame. With phel-lang's let-body inliner (0.46, via #2586 + the inliner-rename fix phel-lang#2622) this now reaches the hot LET-BODIED helpers too: ~45 carry `^:pure` and inline - the per-cell `map/cell` (~600-1400 calls/frame) + `map/wall?`, the per-ray `engine/cell-at` + `projection/wall-px`, `enemy/in-cone?`, and the per-enemy / per-frame AI + physics + render-math helpers - on top of the original let-free `physics/contribution` / `counter-on?`. Byte-identical output (full unit suite + golden hashes unchanged); removes the per-call frame dispatch on the cast / AI / movement paths. Requires `phel-lang/phel-lang` `^0.46` (which ships #2586 + #2622; bumped from `^0.45`). See `docs/performance.md`.
- Render hot-path (issue #262): pin referred frame-math globals (`tex-fade-table`, `bg-cell-cache`, `seam-darken-*`) as frame-level locals in `frame->string` and hoist `floor-flat?` / `vh-1` out of the inner loops, killing per-row and per-cell `Phel::getDefinition()` calls. Byte-identical output; measured -5.9% at 180x40 on no-JIT local PHP.
- Throttled the per-frame terminal-size poll (issue #280): `term-size` forks + execs `stty size` (~1ms + jitter) every iteration, outside the measured adaptive-sleep budget on a 5ms hot path. The four size-polling loops (game loop, end screens, settings, start menu) now sample every 6th frame and reuse the last dimensions, dropping ~5/6 of the forks; the first frame stays eager and a resize is noticed within ~6 frames. Fixed-size frames are byte-identical.
- Render hot path: rewrote the per-column `mix?` flag in `build-wall-sub-bounds` from an `and`/`or` (which Phel compiles to a per-column IIFE closure capturing the whole `frame->string` scope) to a nested `if` ternary that inlines with zero capture - the documented closure-tax fix (`docs/performance.md`). Runs once per scene column on every textured wall column; byte-identical output (golden hashes unchanged).
- Cached the full-resolution weapon-sprite floor gradient in pixel-doubled mode (`render/floor-gradient-full-for`): it was rebuilt every frame via `build-themed-gradient` (~0.2ms) even though it carries no pitch shear and `vh` only changes on resize. A single-slot `vh`-keyed memo (mirroring `gradient-cache`) makes it an O(1) hit. Affects px2 mode only (the slow-hardware / huge-terminal path); output byte-identical.

### Removed

- Removed the entire verticality / multi-tier system (issue #325): a deliberate full flat reset so every level is flat ground again for a clean-slate redesign later. Reverts all the unreleased verticality work - per-cell floor (#232) and ceiling (#235) heights, Z physics with auto-climb / fall damage / handrails (#233/#299), the multi-tier stair rooms + showcase (#237/#298/#310), the enemy tier AI (#301/#311), the cast riser/ceiling accumulators and second riser (#322), the tier render bands + palette + sprite ground-platform pads + cross-tier occlusion (#236/#302/#304), and the floor-height hitscan anchoring (#291/#297). L2-L8 are flat rooms again, each keeping its identity (name, enemy mix, chase speed, keycard lock, weapon drop, rough size); enemies chase flat. The look up/down pitch aim (#231/#240/#243) stays, since that is a camera shear, not verticality. Byte-for-byte identical render (golden hashes unchanged).
- Removed the `Reduced motion` setting: a no-op once the calm 3D view shipped as the default, so the toggle, the persisted `:reduced-motion` key and the `PHEL_DOOM_REDUCED_MOTION` env var are gone. Old settings files carrying the key load fine (the unknown key is ignored).

### Fixed

- Exit doors are now placed RANDOMLY on every level, not just the procgen ones: each level (1-10, including the L10 boss room) gets exactly one exit door at a random wall reachable from the spawn, locked per the level's `:door-lock`, so finding the way out is gameplay and differs every run. Previously the hand-authored layouts (L2-L8, L10) baked a FIXED exit into the ASCII art - and L2's `showcase-layout` shipped with no exit at all, dead-ending the run at the demon room with no door to level 3. `map/place-exit` now owns the exit (layouts define geometry only); a new test asserts every level has exactly one reachable exit whose position varies with the seed, so a missing or fixed exit can't ship again.
- Restart now honors the Minimap setting instead of forcing the overlay on (issue #292): the five restart paths (pause-menu Restart, victory fresh/same, game-over fresh/same) hardcoded `show-map?` to `true` while loop entry seeded it from the persisted `:minimap`. All restarts now derive it via a shared `restart-show-map?` helper. Loop wiring only; render frames are byte-identical.
- Per-frame dt clamped to 100ms before the physics tick (issue #278): after a stall (GC, swap, a debugger break) a delta spike let the un-swept move integrator translate a full move-speed * dt step in one go, tunnelling through 1-thick walls and locked doors and draining every feel timer at once. Real frames (16-50ms) are untouched, so the live tick, FPS and golden frames are byte-identical.
- F9 quick-load no longer reverts settings or persists a stale copy to disk (issue #279): saves drop `:settings` / `:settings-cursor` and a load keeps the live settings, since they are session-global preferences, not run state. Previously a load adopted the baked-in snapshot and the next pause-resume wrote it back permanently.
