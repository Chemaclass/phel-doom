...
- "Quad floor" setting (default off): packs a 2×2 quadrant Block-Elements glyph (`▌▞▖▟…`) per floor cell, adding a 2nd HORIZONTAL ground sample on top of the `▀` half-block's 2× vertical — so the floor texture shows four texels per cell near the camera instead of two. The per-column raycast has no sub-column ray, so the left/right floor basis is lerped 0.25 toward each neighbour column; the four samples quantise to brightest (FG) + darkest (BG) and pick the glyph for that mask. Cells with no horizontal detail (left==right) collapse to the `▀` half-block, so flat / far floor still RLE-coalesces and only near floor pays. Composes with Truecolor. Only with Sub-pixel on, full-detail px1. Cost when on: ~+40-47% CPU, ~+11% bytes/frame (256). Off by default (golden hashes unchanged). Env `PHEL_DOOM_QUAD=1` / `PHEL_DOOM_NO_QUAD=1`.
