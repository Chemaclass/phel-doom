...
- New "Fast walls" performance setting (default off): paints each wall cell as one flat colour instead of a two-sample `▀` half-block, trading wall-interior vertical detail for ~6-14% less render time (the win scales up with screen size) plus ~17% fewer bytes per frame. The wall silhouette / top-bottom edges keep their sub-pixel precision (the seam mixer still runs) and floor / sky / sprites are untouched. Off by default, so the shipped render is byte-identical (golden frame hashes unchanged); persisted with the other options and toggled on the settings page alongside Minimap / High contrast. Aimed at large terminals and slower hardware where the per-cell render loop dominates. The dev/bench override `PHEL_DOOM_FLAT_WALLTEX=1` forces it without touching saved settings.
