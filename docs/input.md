...
- **`reticle-axis(p, len, sens)`** maps a 1-based pointer cell `p` to the reticle's 0-based viewport cell: `clamp(center + (p - 1 - center) * sens, 0, len - 1)` with `center = (len - 1) / 2`. At sensitivity 1.0 it is **1:1** (a 20-cell pointer move is a 20-cell reticle move); a higher sensitivity moves the reticle further per pointer cell. The **clamp** is the key: when the OS pointer sits at or past the window edge, the reticle **pins** at the first / last cell instead of being lost. Returns nil for a nil / `<= 1` axis.
