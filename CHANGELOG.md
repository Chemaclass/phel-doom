...
- Compass top-centre now tints the cardinal letter (E/S/W/N) pointing at the un-picked keycard in the lock colour (blue or red), as a directional hint. When facing == key direction, key colour wins so the cue stays visible. Clears as soon as the player picks up the matching card.
