...
- Auto-calibrated smooth render size. On a big terminal the interpreted-PHP render can't sustain 60fps (the per-cell cost is ~linear in the cell count and dominates the loop; opcache/JIT give nothing measurable on it), so movement felt choppy. The game now renders full-size for a few startup frames (hidden under the intro splash), measures the real render time, then locks the render to the largest size that holds a smooth frame budget (~35-40fps), leaving the surplus terminal as a blank inset border. Half-block sub-pixels keep the smaller render looking sharp. Stable for the session; recalibrates on a terminal resize. A faster machine renders the full terminal and is never capped.
- `--max-cols=N` / `--max-rows=N` flags override the auto-cap: a positive value fixes the render size (smaller inset frame + blank border), and `--max-cols=0` forces the full terminal (uncapped, choppy on slow hardware). Unset = auto-calibrate. Caps only shrink, never grow past the real terminal, and never below a small playable floor. The inset is anchored top-left.
