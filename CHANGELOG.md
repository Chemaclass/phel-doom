...
- The floor you stand on reads as its tier's stone (issue #404, epic #375): standing on a raised platform, the ground the camera casts used to paint the neutral grey base floor no matter which tier you were on - the per-cell height grid never reached the floor-cast path, so your own platform looked like the ground far below. Both emitters now paint the near floor with the SAME warm `tier-riser-tex` stone the tread/riser faces already use, so a platform reads as one continuous stone surface with its steps instead of grey ground under your feet. Reuses the proven truecolor tier-face string path (pack `-2` / `cell-reg`), gated on one frame-constant `elevated?` (`eye-z > 0.5`), so ground-level play and every flat level are byte-identical (dead cond arm; hot-loop closures unchanged at 25) and the elevated floor is even a touch cheaper (one ready string vs the four-sample pack). Colour only for now: the floor geometry stays the z=0 cast (a correct per-tier height projection is the issue's stated bonus) and different tier HEIGHTS still share one slate; per-height hue + off-tier floor cells in view are tracked in #408. New tests pin that a raised-tier room renders warm stone in px1 + px2 while the identical room at ground level stays neutral grey.
- Realistic tiers when climbing + looking down (epic #375): two changes make a staircase read like a modern FPS instead of a flat legibility diagram. (1) The floor cast is now EYE-HEIGHT aware - `build-floor-dperp` scales its numerator by the live `eye-z = 0.5 + player z` (`projection/floor-cast-num`) instead of a baked `0.5`, so standing on a raised tier makes the ground recede faster and drop away, the realistic "look down from a height" view. (2) Tier TOPS (treads/platforms) are now cast in true perspective and textured: a row-remap reuses the base `floor-dperp`/`floor-level` tables at the tread's own height (`tread-height-at` walks `step-bands` for the per-tread `z1`), samples the floor stone, and recolours it through the SAME `tier-riser-tex` LUT the faces use - so a tread and its riser read as one continuous stone surface receding with true distance, replacing the flat `tier-top-ramp` bar (kept only as the degenerate looking-up-a-tall-tier fallback). Risers and treads are retinted from blue slate toward WARM real stone (`r>=g>=b`), still distinct from the neutral grey floor (`r=g=b`) for legibility, and every step keeps its lit lip so stairs stay countable. Flat ground at a level gaze is byte-identical (`eye-z=0.5` makes the numerator equal the old `floor-cast-k` to the bit; hot-loop closures unchanged at 25); `step-bands` widened stride 5->6 for the tread height, px2's tier-top pack `-11` retired in favour of the ready-string `-2`. Ascending staircases get the full perspective-tread treatment; a staircase seen from above shows its correct floor recession but not stepped treads (descending steps emit no `:spans`). Tiered golden frames re-pinned; new unit tests pin `floor-cast-num`, `tread-height-at`, and that a tier renders as warm stone.
- Raised tiers are legible AND look 3D (epic #375): platform tops and stair faces read as a distinct dark slate - never the textured grey floor, so a tier never blends into the ground - but now with real material and depth instead of a flat fill. The step FACE is textured stone: it samples the wall texture but recolours every texel into the dark-slate family (`tier-riser-tex`), so a face shows genuine mortar/stone material yet stays distinct from the grey floor, capped by a lit lip row (`tier-riser-ramp` top) that keeps stairs countable. The tier TOP recedes brighter-near to darker-far (`tier-top-ramp`). Together a platform reads as a stone ledge rather than a 2D rectangle pasted over the world. This lands after walking the trade-offs: flat plain-dark (legible but a sticker), the #373 riser-shade tint (risers read as walls), the #384 / plain-texture attempts (tiers read as floor), and a flat slate gradient (still a sticker at riser height). Face colours aren't stored per column - the emitters sample `wall-tex-px` through `tier-riser-tex` by the row's depth into the face (two `aget`s, the same cost a wall body cell pays); flat levels stay byte-identical and hot-loop closures stay 25. Tiered golden frames re-pinned.
- Staircases render as countable steps (epic #375): a flight of stairs used to collapse into one flat dark block - the painter kept only the nearest riser per column and filled everything behind it with a single tier-top colour, so a platform looked like a floating panel. The renderer now keeps EVERY riser a ray crosses (`compute-riser-shades` appends the farther steps to a per-column `step-bands` list) and shades each one as its own stacked rise-face, the lit top row of each marking the step edge - so stairs stay countable even at a low angle where the treads between them collapse to sub-row. Flat levels are byte-identical (empty `step-bands` = no-op; hot-loop closures unchanged at 25); the farther-band walk runs only on the on-screen staircase area (~+3% on a whole-view stair frame, under the noise threshold). Tiered golden frames re-pinned; a hash-independent test guards that a staircase renders more than one lit step edge.
- Combat vertical aim gate tracks enemy tier height (issue #396, epic #375): a hitscan's see-vs-shoot check (`enemy/vertical-hit?`) now anchors the enemy's feet on its OWN cell floor height, mirroring the renderer's tier feet-anchor (#395), so a shot lands on the drawn body even when the monster stands on a raised platform. At level aim an elevated enemy sits above the fixed screen-centre crosshair (missed until you aim up); aiming up drops the sprite onto the crosshair - what you see is what you can shoot. `combat` threads `(:fz-grid world)` + `eye-z` into `shoot` / `pierce` / `spread-shoot`; their pre-existing shorter arities pass a nil grid + 0.5 eye, so flat-ceiling shipped levels (every enemy on the ground tier) are byte-identical - all existing hitscan tests pass unchanged. New unit tests pin the tier discrimination (level aim hits flat / misses tier; aim-up hits tier / over-shoots flat) and the nil-grid = flat equivalence.

- L2 stands enemies on the staircase (epic #375 showcase): the demon room now pins fixed `:enemy-spawns` - two demons on the raised 0.75 platform and an imp on the 0.5 middle tread - so the enemy tier feet-anchoring (#395) and cross-tier sprite occlusion (#394) are visible in normal play, not only in tests. L2 also now spawns FACING north up the staircase (a new per-level `:spawn-angle`, other levels keep the random facing), so on entry the tier enemies read standing on the platform with the lower imp partly hidden behind the nearer riser - no turning needed - then they chase down the treads at the player. Levels opt in via a per-level `:enemy-spawns` vector of `{:type :x :y [:lives]}`; `build-world` builds them via `new-enemy` with the same depth-scaled `:aggression` as the random roster and appends them (render reads each enemy's cell `fz` on the fly, no stored `:z`). Other levels are unchanged.

- Enemies stand on raised tiers instead of sinking through them (epic #375): the billboard feet anchor now projects the enemy's OWN cell floor height (`enemy-fz`) rather than a hardcoded world z=0, so a monster on a platform or a stair tread stands on that surface. Because the anchor shares the `eye-z = 0.5 + player z` camera height, it also stays correct when the PLAYER is elevated. Flat-floor levels are byte-identical (`enemy-fz` 0, `eye-z` 0.5 collapses to the old `svh/2 + 0.5*wall-px`; golden hashes + full suite green, hot-loop closure count unchanged at 26). Projection carries the enemy's integer cell in `:cell-x`/`:cell-y`; the render reads the height via a bare `:pfz` aget (no closure). All three overlay anchors move together - the body (zone pass), the face glyph (`paint-face-overlay`), and the floating HP digit (`paint-enemy-hp-flashes`) - so nothing detaches on a tiered level. Behavioral test pins that an enemy on a 0.75 platform renders higher than on flat ground. Pairs with the tier/ceiling sprite occlusion so an elevated enemy is both anchored and clipped correctly. (Combat's pitch-based vertical hit gate does not yet track tier height - tracked in #396; flat-ceiling shipped levels are unaffected since enemies spawn on the ground tier.)

- Enemy sprites are occluded by raised tiers and lowered ceilings (epic #375): a monster behind a raised platform or under a hanging ceiling used to paint straight through it - the billboard was gated only against full-height walls (`dists`). Each covered column now also crops the sprite's visible window against the nearer variable-height geometry: `sprite-crop-bot` hides the feet behind a nearer riser (down to its top row), `sprite-crop-top` hides the head under a nearer ceiling drop (up to its bottom row), keyed on `riser-dist` / the new `upper-dist` vs the sprite distance. A fully occluded column paints nothing and no longer occludes pickups or drops a grounding shadow behind the obstacle. The full box still drives sprite sampling so a clipped sprite is cropped, not squished. Flat-floor flat-ceiling levels are byte-identical (empty riser/upper arrays = no-op crop; all golden hashes + full suite green, hot-loop closure counts unchanged at 26/2); only tiered/lowered-ceiling views change. Enemy feet-anchoring to a raised tier's floor height stays deferred.

- Staircase showcase level (#372, epic #375): L2's `showcase-layout` gains a 3-tread staircase (fz 0.25/0.5/0.75 via digits `1`/`2`/`3`) climbing north to a 0.75 platform, viewable from below and above; L2 drops `:walls` so `scatter-walls` spares the treads. Golden frames (below/above/px2) + a physics test walking `:z` up one `fz-step` per tread then holding on the platform. Enemies still render at world z=0 (feet-lift deferred).
- Multi-span column painter (#370, epic #375): renderer paints the `:spans` riser stream from #369 - each column's nearest riser face shades like a side-shaded, `project-height`-projected wall slice, with the tier-top floor visible beyond it. Ascending-distance order IS the row clip (no occlusion pass). Flat worlds unaffected (`:spans` empty, one `aget` sentinel, zero per-cell branching; `main.php` closures dropped). Tier top uses the flat gradient for now; `projection/tier-top-row-dist` added for a future textured pass. Golden tests on a staircase fixture.
- Upper wall-slice painter for lowered ceilings (#387, epic #375 Phase 3): renderer draws the `:uspans` ceiling-drop stream - each drop paints a wall-shaded band from the ceiling top down to the lowered plane (visual mirror of #370). `compute-upper-shades` projects `cz0`/`cz1` via `project-height` once per column; per-cell paint stays one `aget` (hot-loop closures unchanged 26/2), band wins over sky. `projection/ceiling-top-row-dist` added (unit-tested, wired for a future textured pass). Flat-ceiling worlds byte-identical; a synthetic lowered-ceiling room exercises px1 + px2.
- Window-gap passability + shoot-through (#388, epic #375 Phase 3, completes variable ceilings): a lowered ceiling now blocks movement and shots. `try-move` gains a headroom gate - walk-through only when the opening (`ceiling-z - floor-z`) clears `player-height` (one `fz-step`, 0.25); a lower lintel blocks walking but stays see-through. Combat routes hitscan through `cast-shot-dist` (gap-aware `cast-ray`): a ceiling at or below muzzle height (`0.5 + player z`) stops the shot, higher is shoot-through. `player-height` = `fz-step` keeps every flat-ceiling level neutral (L2's fz-0.75 platform still walkable/shootable); floor risers don't block shots. Physics + combat unit tests; enemy headroom parity deferred.
- Player `:z` + floor-height step-up rule (#371, epic #375): player record gains `:z` (floor height under the feet); `try-move` allows a rise of at most one `fz-step` (0.25) above `:z` (bigger blocks like a wall) and snaps `:z` to the destination `fz` (stepping down any depth allowed, no fall lerp). Enemies get the same rule so monsters climb stairs. Flat levels: no-op.
- Upper-span cast for ceiling drops (#386, epic #375 Phase 3): DDA tracks ceiling height (`:pcz` from #385) and emits a flat `:uspans` stream where the ceiling DROPS (mirror of #369's risers, stride-8, `cz1 < cz0`); a raised floor + lowered ceiling cell is a window gap (both spans). `do-cast` picks one of three march variants per frame on `:fz-flat?`/`:cz-flat?`: both-flat legacy, tiered-floor #369 march, lowered-ceiling combined - the first two byte-identical, only lowered ceilings pay (~+53% cast, no shipped content). Render ignores `:uspans` until #387.
- Per-cell floor-height data layer (#368, epic #375): worlds carry `:fz-grid` (quantized 0.25 steps) + flat PHP twin `:pfz` (`y*width+x`); layouts author tiers via digits `1`/`2`/`3` (fz 0.25/0.5/0.75); `map/floor-z` returns 0.0 out of bounds. Data only - every shipped level flat, saves round-trip heights without a version bump.
- Per-cell ceiling-height data layer (#385, epic #375 Phase 3): worlds carry `:cz-grid` (default 1.0) + flat twin `:pcz` (`y*width+x`) + `:cz-flat?`, mirroring #368; `map/ceiling-z` reads 1.0 out of bounds; layouts author lowered ceilings via chars `a`/`b`/`c` (cz 0.75/0.5/0.25). Saves round-trip `:cz-grid` without a version bump. Data only - shipped levels full-height.

### Performance

- Cut per-step dispatch in the `cast-ray` + `los-clear?` DDA paths (epic #354, the #357 follow-up): both inline the cell probe via an `^array` `row` binding (native `$row[k] ?? 1`, OOB = wall) and bind `max-depth`/`dda-inf` once as locals; `cell-at` removed (no callers left). Byte-identical (full suite 3182 + golden hashes); -49% on a 360-ray `cast-ray` sweep, -42% on `mark-visible-cells`; `cast-frame` untouched.
- Trimmed per-frame pickup-paint allocations (#348): the 8 keycard/weapon-pickup passes now guard on `(seq …)`, skipping the filter, vector build, and `paint-pickups-into` on the common zero-pickup frame; the two source lists bind once instead of re-reading `world` 8x. `pulse` gains `^:pure`. Byte-identical; new test pins a visible pickup still paints.
- Cut per-step dispatch + per-column recompute in the cast loop (#347): hoist frame-fixed `cx0`/`cy0` + the four boundary offsets out of the per-column loop; inline `cell-at` via `^array` subscript; bind `max-depth`/`dda-inf` once. Byte-identical (full suite 2973 + golden hashes); -55%/-63%/-66% on `cast-frame` at 80/120/180 columns.
- Eliminated the per-column closure tax in the cast loop (#345): the DDA march + `wallx` branch sat in `let`-value position (each an IIFE capturing ~40 locals, ~720/frame at 240 cols); both moved to statement position into a reused 5-slot `hit-reg`, `wallx` now a pure ternary. Byte-identical; -39% on `cast-frame`. `engine.php` drops from 5 `function() use(` to 2. See `docs/performance.md`.
- Quad-detail floor cells skip quantise on cache hits (#346): 256-code quads key `quad-cell-cache-256` on the raw `(tl tr bl br)` tuple and return before any quantise; truecolor/mixed quads keep the quantize-keyed cache (a raw 4-channel key overflows 64-bit). Byte-identical; `:quad` path only (off by default).
