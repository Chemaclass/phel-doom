...
- Mouse aiming and click to shoot (issues #246, #318, #320, #288, #324): a movable `+` reticle follows the pointer 1:1 inside the 3D viewport, and the gun fires an off-centre hitscan at whatever enemy the reticle covers instead of a fixed screen-centre axis. Pushing the reticle into the outer edge band pans the camera (yaw + pitch) to aim beyond the view; the central region is pure 1:1 aim. A terminal cannot lock, warp or hide the OS pointer (iTerm2 included, #313), so the reticle is the pointer's absolute position clamped to the viewport - it pins at the border and edge-pan keeps turning rather than freezing mid-screen. Left-click fires (hold to spray auto-fire weapons), via xterm SGR mouse reporting. A Mouse on/off toggle and a Sensitivity slider are added and persisted; with the mouse off the reticle pins to centre and play is byte-for-byte the keyboard-only build (golden fixture unchanged). On by default, keyboard controls unchanged. The reticle row is informational for now - enemies are full-height billboards, so the column picks the target.
- Geometric mouse Sensitivity curve (issues #275, #306): the slider follows `3 ^ ((pct - 50) / 50)` - 50% = neutral 1.0x (saved settings unchanged), 0% drops to ~0.33x for fine aim, 100% reaches 3.0x for fast flicks - scaling the edge-pan camera-swing rate. The reticle itself always tracks the pointer 1:1 regardless of sensitivity, so the slider tunes how fast the camera swings when the reticle is pushed into the outer edge band, not where the `+` lands.
- Mouse pointer hidden over the 3D view (issue #295): `mouse-enable` sends XTSMPOINTER `\e[>3p` to hide the OS pointer (not just the text caret) and `mouse-disable` restores it with `\e[>1p`. Works on terminals that implement XTSMPOINTER; gated on the Mouse setting; render frames byte-identical.
...
- Mouse reticle is 1:1 with the pointer at every sensitivity (issue #324 follow-up): the reticle position no longer scales with the Sensitivity slider (it previously drifted the `+` off the cell the mouse pointed at, away from 50%). `reticle-axis` now maps the pointer cell straight through (`clamp(p - 1)`), so the crosshair sits exactly where the mouse points and the gun - which aims at the `+` - is 1:1 with it. Sensitivity now scales only the edge-pan camera-swing rate.
