...
- Compiler call inlining (issue #166): `^:pure` lets opt level 2's `CallInliner` splice pure helpers at their call sites instead of dispatching through a PHP frame. With phel-lang's let-body inliner (dev-main #2586, fixed in phel-lang#2622) this now reaches the hot LET-BODIED helpers too: ~45 carry `^:pure` and inline - the per-cell `map/cell` (~600-1400 calls/frame) + `map/wall?`, the per-ray `engine/cell-at` + `projection/wall-px`, `enemy/in-cone?`, and the per-enemy / per-frame AI + physics + render-math helpers - on top of the original let-free `physics/contribution` / `counter-on?`. Byte-identical output (full unit suite + golden hashes unchanged); removes the per-call frame dispatch on the cast / AI / movement paths. Requires `phel-lang/phel-lang:dev-main` (bumped from `^0.45`). See `docs/performance.md`.
