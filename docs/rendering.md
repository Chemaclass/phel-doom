...
- `riser-shade`: a PLAIN dark slate cell (`tier-riser-fill`, `48;2;32;36;52`). The step face reads as a flat dark border distinct from both the walls and the textured grey floor, so a stair edge never blends into either. This is deliberately colour-flat: no distance/wall-index shading and no per-tier tint (the earlier #373 `riser-shade-level` approach sampled the wall shade table, which made risers read as walls); legibility of the step comes from the face-vs-surface contrast, not from a shade gradient. Riser faces are usually one or two rows tall.
