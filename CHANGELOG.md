...
- Staircases render as countable steps (epic #375): a flight of stairs used to collapse into one flat dark block - the painter kept only the nearest riser per column and filled everything behind it with a single tier-top colour, so a platform looked like a floating panel. The renderer now keeps EVERY riser a ray crosses (`compute-riser-shades` appends the farther steps to a per-column `step-bands` list) and paints each one: a dark rise-face stacked up the plateau, capped by a lit front-edge lip (`tier-lip-fill`, one brighter-slate row) at the top of each step. The lip keeps stairs legible even at a low angle where the treads between them collapse to sub-row. Stays in the dark slate family (a highlight, not a white line). Flat levels are byte-identical (empty `step-bands` = no-op; hot-loop closures unchanged at 25); the farther-band walk runs only on the on-screen staircase area (~+3% on a whole-view stair frame, under the noise threshold). Tiered golden frames re-pinned; a hash-independent test guards that a staircase renders more than one dark face plus the lip.
- Legible raised tiers (epic #375): platform tops and stair faces now paint as a PLAIN dark slate distinct from the textured grey floor - a darker step face under a lighter tread top - so stairs and high platforms read at a glance instead of blending into the ground. This supersedes the earlier same-cycle riser-shade tint (#373) and floor-texel tier-top sampling (#384), both of which made a tier look like the floor (which was the legibility problem itself). It is also strictly cheaper: the per-column `tier-top-row-dist` inversion + fog `pow` are gone (`main.php` hot-loop closures 26 -> 25). Flat levels stay byte-identical (empty riser arrays = no-op); tiered golden frames re-pinned.
