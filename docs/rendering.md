...
- `step-bands`: the per-column list of the FARTHER step faces (every riser beyond the nearest), stride 3 `[top bot dist ...]` near-to-far. Absent for a single-step column. `:spans` for a column are contiguous, so this is built with a fetch-mutate-store (PHP copies nested arrays by value; the store writes the mutated copy back). Two pure helpers read it in the row loop: `row-in-farther-face?` (is this row inside a farther face) and `row-at-farther-face-top?` (is this row the lit front edge of one).

The row loop (both the px1 half-block path and the px2 pixel-doubled path) tests, in order: sky, then the nearest step - a lit lip at `riser-top`, a dark face `(riser-top, riser-bot)`; then the plateau above `riser-top`, subdivided per row into a farther step's lip (`row-at-farther-face-top?`), a farther step's dark face (`row-in-farther-face?`), or the light tread/platform surface between them; then it falls through to the ordinary floor/wall logic unchanged. On a flat world `:spans` is empty, so every column's `riser-top` aget reads `nil`, `has-riser?` is false, `step-bands` is absent, and all the new branches short-circuit as dead comparisons (see "Perf" below).
