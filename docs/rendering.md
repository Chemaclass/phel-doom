...
Issue #370 (epic #375) consumes the `:spans` riser-event stream #369's cast produces (see raycaster.md "Multi-span cast"). `compute-riser-shades` scans the flat stream once per frame. `:spans` arrives ordered by ascending distance within a ray (one ray per column), so a column's events are contiguous and near-to-far: the FIRST is the nearest riser, the rest are the higher steps stacked behind it up the staircase. A staircase must render as countable steps, not one flat block (#375), so every event is kept - the nearest fills the fast-path fields the emitters read directly, and each farther one appends to a per-column band list (`step-bands`).

For a column with a riser, `compute-riser-shades` returns:

- `riser-top` / `riser-bot`: the NEAREST riser face's screen rows, via `project-height` at the riser's fish-eye-corrected distance with `z1` (tier top) and `z0` (tier bottom) - the same projection kernel walls use, so a riser reads as a wall slice, not a special case.
Face and tier-top COLOURS are not returned per column: the emitters index two global gradient ramps by the row's depth into the face / plateau, so a surface reads as a shaded 3D plane, not a flat constant. The distinct dark-slate hue is kept (never the busy floor texture, so a tier never blends into the ground), but it now has depth (#402).
- `riser-dist`: the corrected distance, for the same z-buffer role `dists` plays for walls (enemy/pickup occlusion compares against it). Nearest riser only - a sprite is occluded by the closest tier.
- `riser-u` / `riser-level`: the nearest face's texture column (0..63, from the span's `wallx`) and fog level (0..23, the same wall gamma-falloff + EW side bonus `compute-wall-shades` computes, haze-shifted). The face is sampled from `wall-tex-px` so a riser reads as real textured stone (#404).
- `step-bands`: the per-column list of the FARTHER step faces (every riser beyond the nearest), stride 5 `[top bot dist u level ...]` near-to-far (each band carries its own texture coords). Absent for a single-step column. `:spans` for a column are contiguous, so this is built with a fetch-mutate-store (PHP copies nested arrays by value; the store writes the mutated copy back). Two pure helpers read it: `farther-face-band` returns the base index of the band a row lands in (so the emitter can pull its `top`/`bot`/`u`/`level`), or -1; `farther-face-vstep` derives the row's depth into that face from the same scan.

The row loop (both the px1 half-block path and the px2 pixel-doubled path) tests, in order: sky, then the nearest step face `[riser-top, riser-bot)` - its top row (`row == riser-top`) is the lit lip (`tier-riser-ramp` index 0), every row below samples the warm-stone wall texture (see **Riser face**); then the plateau above `riser-top`, where each row is either a farther step's face (`farther-face-band >= 0`, its own lip + texture) or a perspective-cast textured tread (see **Tier-top floor**); then it falls through to the ordinary floor/wall logic unchanged. On a flat world `:spans` is empty, so every column's `riser-top` aget reads `nil`, `has-riser?` is false, `step-bands` is absent, and all the new branches short-circuit as dead comparisons (see "Perf" below).

**Riser face** is textured stone (#404): every row below the lip samples `wall-tex-px` at `texRow = min(63, (row - face-top) * 64 / face-height)`, column `u`, and maps the texel through `tier-riser-tex` - a `[level][texel]` table (24×256, same shape as `tex-fade-table`) that recolours each grey wall texel into a warm real-stone family (`luma -> (luma*0.72+34, *0.68+30, *0.62+26)`, so `r>=g>=b`, lerped toward the warm shadow-base `34,30,26` by `level/23` so distance haze still applies but never leaves the stone). This keeps the texture's PATTERN (a face shows material, not a flat sticker) but in a warm rock/brick hue that stays distinct from the NEUTRAL grey floor (`r=g=b`), so a riser never blends into the ground - the failure mode of the first cut (#404 iter 1) which sampled the plain grey texture and read as literally the floor (the same #384 problem, moved to the face). The top row of every face (`row == face-top`) stays the flat bright `tier-riser-ramp[0]` lip (`48;2;150;138;116`), the countable-stairs edge that reads even when a face is 1-2 rows tall. The nearest face reads `riser-u`/`riser-level`; farther faces read their own from `step-bands`. (Earlier iterations were blue-slate #402/#404; the warm retint is the "real stone like DOOM" pass.)

**Tier-top floor** is the raised-tier ground: the treads between steps and the platform capping the staircase, visible between the far wall's bottom and the nearest riser's top. Each tread is now cast in TRUE PERSPECTIVE and textured (the "realistic tiers" pass), so a raised tier reads as real receding stone you climbed rather than a flat bar. A tread sits at world height `z-tread` (from `tread-height-at`, which walks the nearest riser + `step-bands` to find the step a tread row caps); by the height-aware floor-cast identity, a tread at height `z` shows at screen row `row` the world point the FLAT ground shows at the remapped row `rp = (flr-center - 0.5) + eye-z/(eye-z - z) * (row + 0.5 - flr-center)`. So the tread reuses the base `floor-dperp` / `floor-level` tables at `rp` (no per-cell `pow`, px2-safe), samples `floor-tex-px` at `player + dperp * (floordx, floordy)`, and recolours the texel through the SAME `tier-riser-tex` LUT the faces use - tread and riser read as one warm stone material, distinct from the neutral grey floor (#384), fogged by true distance. This is algebraically `projection/tier-top-row-dist` (unit-tested, and the reconcile assertion pins the equality), so that kernel is effectively wired now. A tread at or above eye height (looking UP a tall tier, `eye-z - z <= 0.05`) cannot be floor-cast - it falls back to the flat `tier-top-ramp` warm-stone gradient, which is now ONLY that degenerate fallback. In pixel-doubled mode the tread is whole-cell too, stored as a ready string via pack `-2` (the old pack `-11` tier-top kind is retired).

**Sprite anchoring**: enemy billboards anchor their feet at the occupied cell's floor height (`:pfz`), so a monster on a raised tier stands on the platform (issue #375, see "Feet anchoring" below). All three anchor sites move together - the body in `main.phel`'s zone pass, the face glyph in `paint.phel`'s `paint-face-overlay`, and the floating HP digit in `paint-enemy-hp-flashes` - each reading `enemy-fz` via a bare `:pfz` aget with the same `eye-z = 0.5 + player z`.

**Eye-height floor recession**: the base ground plane (`build-floor-dperp` / `-sub`) is cast with the live camera eye height `eye-z = 0.5 + player z` (`floor-cast-num proj-dist eye-z 0.0`), not the old baked `0.5`. Climbing a staircase raises the eye, so the floor recedes faster and the ground drops away - the realistic "look down from a height" view. At `eye-z = 0.5` (level gaze on flat ground) the numerator equals the old `floor-cast-k` constant to the bit, so flat levels stay byte-identical. (Descending steps emit no `:spans`, so a staircase seen from ABOVE renders its correct floor recession but not stepped treads - the ascending view gets the full perspective-tread treatment.)

**Tier floor tint + recession (the floor you stand on, #404 + #416b)**: when the player stands on a raised tier (`elevated?` = `eye-z > 0.5`), the near ground the camera casts IS that tier, so both emitters paint it the same warm `tier-riser-tex` stone the tread/riser faces use - via the tier-face ready-string path (pack `-2` in px2, a `cell-reg` string in px1), one representative floor texel per cell. So a platform reads as one continuous stone surface with its steps instead of the neutral grey base floor. The near floor's GEOMETRY now casts at the tier's own height too (#416b): the floor you stand on is 0.5 below the eye at any tier height, so a `floor-dperp-tier` / `-sub` table built from the plain height-independent `floor-cast-k` / `-sub` const (reused directly, not re-derived as `eye-z - player-z`, so nearby-float subtraction noise can't leak into which texel is sampled) makes the plateau recede at the grounded rate instead of the too-fast `eye-z / z=0` rate. That table is gated on `elevated?` (and further on the render mode that consumes it - px1 reads the cell pair, px2 the sub pair), so ground-level play and every flat level build NEITHER table and stay byte-identical. Descending / off-tier stepped floor geometry (symmetric `:spans`) is still deferred - #416 part (a) / #408. Because `tier-riser-tex` is fog-indexed (not per tier NUMBER), every raised tier shares one slate - distinguishing different tier HEIGHTS by hue, and tinting off-tier floor cells visible in the same column, are tracked in #408. `elevated?` is a frame constant, so ground-level play and every flat level are byte-identical (the new floor `cond` arm is a dead comparison there; the flat-floor dev override (`floor-flat?`, env `PHEL_DOOM_FLAT_FLOOR` only, not the in-game "Low detail" setting) takes one fixed `tier-riser-ramp` stone; hot-loop closures unchanged at 25).

### Perf

Flat worlds pay near-zero extra cost. `:spans` is empty, `compute-riser-shades` returns empty arrays in one pass over zero events, and the row loop's riser lookup is a single inlined `aget` + `nil`-compare per cell (no function dispatch, no `or`, no branching in binding-value position) - the closure-tax discipline (docs/performance.md) holds: no new closures on the flat path (built `main.php` closure count DROPPED from 34 `function() use(` on the pre-#370 baseline `1fb15c0` to 25, because several existing per-frame bindings were rewritten as `if`-ternaries in passing; the multi-step + shading + texture work (#375/#402/#404) added `farther-face-band` / `farther-face-vstep` as separate pure fns and its plateau branches (incl. the texture-sample `let`s) as statement-position forms, so the count stayed 25). The only residual flat-path cost is the handful of extra `cond` clauses evaluated per cell, which short-circuit on the `nil` sentinel. `farther-face-band` is called only for the PLATEAU rows of a riser column - a per-cell function dispatch bounded by the small on-screen staircase area, dead everywhere else; the per-cell riser texture sample is two `aget`s (`tier-riser-tex[level][ wall-tex-px[...] ]`) into module-constant tables, the same shape a wall body cell already pays.

Perf gate (measured, `frame->string` micro-bench, deterministic worlds, no-JIT local): flat `build-world 1` at 180x40 renders at ~6.9ms/frame vs ~6.6ms on the pre-#370 baseline - **+~5%**, inside measurement noise (the baseline's own runs span 6.5-7.0ms). The showcase staircase from below at 120x30 (the worst case for the plateau band-walk, whole view stairs) renders at ~6.2ms/frame vs ~6.0ms for the single-step painter - **+~3%**, under the 5% noise threshold; a flat frame is unchanged (dead nil-check). An earlier draft anchored the riser lookup in a `riser-top-at` FUNCTION called per cell, which cost +15-23%; inlining the `aget` recovered it. The staircase fixture stays comfortably under the 16ms hard ceiling. This per-cell cost mirrors the enemy-sprite zone checks, which are likewise evaluated on every cell and gated by an unset-array sentinel.

The perspective-tread pass (realistic tiers) adds a bounded per-tread-cell cost: `tread-height-at` (a short scan of `step-bands`, one entry per visible step) plus the row-remap (one divide + one round + a handful of `aget`s reusing the base `floor-dperp` / `floor-level` tables - deliberately NO per-cell `pow`, the texel fog comes from the existing table). Measured on the worst case (a whole-view staircase at 120x30, using the staircase-minus-flat delta in the same process to cancel per-process baseline drift), the tread cast adds ~0.3ms over the pre-perspective painter (~1.3ms staircase tax vs ~1.06ms); total worst-case stays ~7ms/frame, well under the 16ms ceiling. Flat worlds are byte-identical (empty `:spans`, dead branch), so non-tiered levels pay exactly zero, and the built `main.php` closure count stays 25.
...
Same discipline as the riser painter: flat-ceiling worlds pay near-zero cost (`:uspans` empty, `compute-upper-shades` returns empty arrays in one pass, the per-cell upper lookup is a single inlined `aget` + `nil`-compare, no binding-value closures). The built closure counts stay low: `out/phel_doom/io/render/main.php` is at 25 `function() use(` sites (down one after the tier-top texturing revert dropped its per-column `tier-top-row-dist` inline), `core/engine.php` at 2. Only lowered-ceiling columns (rare - no shipped level has them yet) evaluate the paint band.
...
**Sprite mode (default):** enemies render as baked Freedoom (BSD) billboards. `enemy_sprites_data` holds a native-resolution xterm-256 grid per sprite-id (`{:w :h :px}`, -1 transparent); `enemy-sprite/sprite-for-type` maps each `:type` to one (pinky + spectre reuse `:demon`). Box width comes from the sprite's NATIVE aspect (`half-width = round(h * sw / sh)`, terminal cells being ~2:1 tall) so the monster is never squished by the glyph hitbox. The zone pass stashes per covered column (depth-gated `d < dists[c]`) the billboard box + sprite ref in `e-stop/e-sbot/e-sx/e-spx/e-sw/e-sh` WITHOUT clobbering the wall `tops/bots`, so transparent pixels show the real wall behind. Beyond the full-height wall gate, each column's visible window is also cropped against the NEARER variable-height geometry (epic #375): `sprite-crop-bot` pulls the box bottom up to a nearer riser's top row (a raised tier between camera and enemy hides the feet), and `sprite-crop-top` pushes the box top down to a nearer ceiling drop's bottom row (a lowered ceiling hides the head), keyed on `riser-dist` / `upper-dist` vs the sprite distance. The full box still drives sprite-y sampling while `e-vtop/e-vbot` gate which rows paint, so a tier-clipped sprite is CROPPED at its true pixels, not squished (the #315 crop-not-squish contract). A fully occluded column collapses the window and paints nothing - and does not update `edists`, so a hidden enemy neither occludes a pickup behind it nor drops a grounding shadow. On a flat-floor flat-ceiling world the riser/upper arrays are empty, both crops are no-ops, and the window is the full box (byte-identical). The hot row loop computes the wall/sky/floor base cell, then overlays `enemy-sprite-cell`, which stacks TWO sprite sub-pixels per cell via a half-block (`▀` fg=top bg=bottom) for 2x the vertical resolution; an edge cell with one opaque sub-pixel renders a solid `█` of that colour (clean silhouette, no halo against any background). `sprite-col-x` (column to sprite-x) is precomputed once per column; only the sub-pixel `sy` pair is per-cell. The crosshair centre-cell resolver mirrors the same priority.
...
**Feet anchoring.** The renderer stands each billboard on its feet, not centred on the horizon. The enemy zone pass projects the floor row for the enemy's own CELL floor height (`enemy-fz`, read via a bare `:pfz` aget): `round(svh/2 - (enemy-fz - eye-z)*wall-px) + pr`, and sets the body to span from head to feet on that row (issue #375). So a monster on a raised tier stands ON the platform instead of sinking to world z=0, and because `eye-z` is `0.5 + player z`, the anchor also stays correct when the PLAYER is elevated. On flat ground (`enemy-fz` 0, `eye-z` 0.5) this is exactly the old `svh/2 + 0.5*wall-px`, so flat levels are byte-identical. The projection is written inline via `wall-px` (not a `project-height` call) and the `:pfz` read is a bare aget (not a `floor-z` call), both to keep the built-file closure count at its documented baseline. The grounding shadow drops on the foot row; the face glyph and floating HP digit ride the same anchor. `scene-pd` is halved in pixel-doubled mode, so the projection scales automatically. The golden hashes in `render-cache-test/test-frame-bytes-pinned` are unchanged because the golden fixture has no enemy on-screen.
