...
| Reduced motion | bool | on / off | Accessibility / photosensitivity. When on, gates the strobing horror beats: the lights-flicker scanlines are dropped, the low-health heartbeat edge and the berserk border hold steady instead of pulsing, and the one-shot jump-scare face flash is suppressed. Essential single-shot feedback (the directional hit-vignette, kill flash) is kept. `PHEL_DOOM_REDUCED_MOTION=1` forces it on regardless of the saved value. |
