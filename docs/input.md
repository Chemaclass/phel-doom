...
A terminal **cannot lock, warp, or hide the pointer** on most terminals (iTerm2 included, #313): there is no portable way to recenter the OS cursor. A raw delta-only mouselook would lose the pointer the instant a fast flick carries it off the window - the terminal stops reporting and the turn freezes. So `mouse-aim` (pure) tracks the pointer's **absolute position**, computes the **delta** from the previous frame, applies that delta as a 1:1 camera turn, and when the pointer hits an edge (where delta stops), `edge-pan-rate` sustains the turn - aiming never freezes mid-screen.
...
- **Held at an edge keeps panning.** The terminal sends motion reports only when the pointer moves, so a pointer held still off-screen yields **no new bytes**. The edge-pan reads the **cached** `:pos` every frame, so an empty `keys` string still applies the pan from the last known position - the camera keeps turning while the pointer rests at (or past) an edge, so the player can aim beyond the current view.
- **Pointer-drift proof.** Pointer in the interior = pure delta turn, no sustained pan; move it to a border = delta halts but edge-pan sustains; move it back = delta resumes. When the OS pointer wanders out of the window it stops reporting but `:pos` stays cached, and edge-pan keeps the camera turning - **recoverable**, never the mid-screen freeze of raw delta-only mouselook. Re-entry just resumes delta tracking from wherever the pointer reappears.
