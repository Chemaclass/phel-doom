...
- Level generation is 2.2x faster: 42.7 ms per level down to 19.3 ms, and up to 78 ms on the biggest level before. The two passes that dominate it - dropping the exit at a random reachable wall, and sealing any floor pocket the scattered wall blobs cut off - both flood the floor and then scan every cell, and both read the grid through the persistent vector accessor while keying their seen-set by a freshly built `"x:y"` string per neighbour test. Reading through a PHP-native mirror and keying by a flat index takes `place-exit` on a 64x40 grid from 57.9 ms to 18.1 ms and the pocket seal from 58.1 ms to 18.4 ms. Levels are unchanged: the flood consumes no rng and the scan order is the same, so all 210 worlds (10 levels x 7 seeds x 3 difficulties) hash identically.
