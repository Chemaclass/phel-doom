...
**Feet anchoring**: A sprite is anchored by its feet on the enemy's own cell floor row (`round(svh/2 - (enemy-fz - eye-z)*wall-px) + pr`, issue #375), not centred on the horizon; on flat ground (`enemy-fz` 0, `eye-z` 0.5) this reduces to `svh/2 + 0.5*wall-px`. The renderer stands each billboard on that row and draws the body upward by its pixel height, so a monster on a raised tier stands on the platform rather than sinking to z=0. The grounding shadow, the face glyph, and the floating HP digit all derive from the same foot row. Combat's vertical hit gate (`aim-pr` / `sprite-half-rows`) is pitch-based and does NOT yet account for tier height, so a shot at a tier-elevated enemy uses the flat-anchor gate; on flat-ceiling shipped levels this is unchanged (all enemies spawn on the ground tier), and closing the gap is tracked in issue #396.
