...
- Compiler call inlining (issue #166): annotate the let-free per-frame movement helpers `physics/contribution` and `physics/counter-on?` with `^:pure` so opt level 2's `CallInliner` splices them in at their call sites instead of dispatching through a PHP frame. Output is byte-identical (tests + golden hashes unchanged); the win is small (a handful of calls per frame in the movement step, off the cast/render hot path) and the higher-frequency `map/cell` / `map/wall?` stay on dispatch because their `let` bodies are outside the inliner's whitelist. See `docs/performance.md`.
