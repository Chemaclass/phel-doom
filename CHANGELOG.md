...
- Raised tiers are legible AND look 3D (epic #375): platform tops and stair faces paint as a distinct dark slate - never the textured grey floor, so a tier never blends into the ground - but now with depth instead of a flat fill. The riser face gradients top-lit to shadow-base down its height (`tier-riser-ramp`, its lit top row doubling as the step lip), and the tier top recedes brighter-near to darker-far (`tier-top-ramp`), so a platform reads as a lit 3D surface rather than a 2D rectangle pasted over the world. This replaces the earlier same-cycle attempts that each traded one problem for another: the flat plain-dark fill (legible but a flat sticker), the #373 riser-shade tint (risers read as walls), and the #384 floor-texel tier-top (tiers read as floor). Colours are not stored per column - the emitters index the two global gradient ramps by the row's depth into the face / plateau, one `aget`, so flat levels stay byte-identical and hot-loop closures stay 25 (~+0.5% on a stairs frame, noise). Tiered golden frames re-pinned.
