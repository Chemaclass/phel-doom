...
- Enemy sprite bodies are no longer near-black silhouettes against dark walls: the bake tool now applies a per-channel gamma=0.50 (square-root) lift via `tone-lift` before quantization, pushing dark-brown shadow bands (source lum ~15-40) into distinguishable xterm-256 codes while preserving hue (all channels lift proportionally, so reds and yellows stay red and yellow). Grayscale ramp codes (232-255) receive an additional +3-step shift post-quantization (+30 lum) via `gray-shift`, separating the main body shadow tone from wall grays without affecting the coloured cube codes. The textured sprite fog is now applied at half-strength vs walls (`sfade = (1 - body_fade * 0.5) * 23`), so shading bands survive at mid/far range without washing out; the wound-damage tint contribution is not halved, preserving gameplay health-state readability. Near imp now shows 4+ distinct shading bands (black outline, dark-gray body, warm tan torso, red crest, yellow legs); far imps stay hazy with no glow.
...
- Pickups (health, armor, ammo, soulsphere, berserk, invuln, backpack, keycards, weapon pickups) now render as real Freedoom item sprites instead of flat coloured quads. The existing `pickup-sprites` baked data and `blit-sprite-into` path were already present but gated behind a 7-row LOD threshold that only fired at d < 1.8 (point-blank). The threshold is now `max(2, 2*lodr)` with billboard height `min(16*lodr, 0.55*sprite_h)`, engaging sprites at any distance where the item renders >= 2 rows tall. The coloured-glow fallback is kept for extreme range or `PHEL_DOOM_NO_SPRITES=1`. Half-strength fog (`df*0.5`) applied consistently via `blit-sprite-into`. Pickup sprites are baked without the gray-ramp-shift (+3 step) that enemy sprites use - item art is already in the bright lum 88-168 range and the shift was washing codes toward near-white; a separate `mip-entry-pickup` / `rgb->256-pickup` path applies only the gamma=0.50 tone-lift.
