...
- **Sub-row wall seams.** Textured columns place the wall top/bottom at sub-row precision (`build-wall-sub-bounds`, half a terminal row - the full-detail seam precision) and the boundary cells mix sky / wall / floor codes inside their 4-sample pack, so receding wall silhouettes step in half-row increments instead of whole 2-row scene cells. The wall's top sub-row, its bottom two sub-rows and the floor's first sub-row pull a few grayscale steps darker - a thin border line that traces the wall/floor and wall/sky limits along the exact diagonal. Interior wall cells keep a straight 4-sample fast path; only the one or two boundary cells per column pay the per-sub-row cond.
