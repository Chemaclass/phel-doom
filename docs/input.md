...
- `\e[?25l`: hide cursor (also re-asserted every frame by `render!` so the caret can't resurface mid-session behind streaming mouse reports or after a resize - see [Aim point](#aim-point-the-movable-crosshair-hidden-os-pointer))
...
Modern-FPS controls in the terminal: a `+` aim crosshair follows the mouse 1:1, the gun fires toward wherever it points, and pushing the reticle to a screen edge pans the camera (#324). **Additive and backward-compatible** - every keyboard binding is unchanged; the mouse is an extra input path, on by default and toggleable via the Mouse setting. With the mouse off the reticle pins to centre and the game plays exactly like the keyboard-only build.
...
### Movable aim crosshair (issue #324)

A terminal **cannot lock, warp, or hide the pointer** on most terminals (iTerm2 included, #313): there is no portable way to recenter the OS cursor. A raw 1:1 **mouselook** would lose the pointer the instant a fast flick carries it off the window - the terminal stops reporting, the aim freezes, and the OS pointer races on. So `mouse-aim` (pure) makes the reticle the pointer's **absolute position clamped to the viewport**, and pans the camera from the **edge band** - aiming never freezes mid-screen.

`controls/mouse-aim` parses every report in the drained byte string, **caches the latest pointer position** (`:pos`, carried forward when no report arrives), and returns `{:aim-col :aim-row :yaw :pitch :fire? :fire-held? :pos}`. The reticle is the pointer's cell clamped to the viewport; the camera edge-pan **rate** is a function of how deep the reticle sits in the outer band, applied **every frame**:

```
aim-col =  reticle-axis(pointer_x, cols)
aim-row =  reticle-axis(pointer_y, rows)
yaw     =  edge-pan-rate(aim-col, cols, mouse-yaw-rate-max)   * sensitivity
pitch   = -edge-pan-rate(aim-row, rows, mouse-pitch-rate-max) * sensitivity
```

- **`reticle-axis(p, len)`** maps a 1-based pointer cell `p` to the reticle's 0-based viewport cell: `clamp(p - 1, 0, len - 1)`, an exact **1:1** tracking with the pointer (a 20-cell pointer move is a 20-cell reticle move) that is **independent of Sensitivity** - the `+` always sits exactly under the cell the mouse points at. Returns an int (nearest whole cell) so the reticle glyph, the scene highlight, and the shot share one integer column. The **clamp** is the key: when the OS pointer sits at or past the window edge, the reticle **pins** at the first / last cell instead of being lost. Returns nil for a nil / `<= 1` axis.
- **`edge-pan-rate(r, len, max-rate)`** for a 0-based reticle cell `r`: the inner **centre region** (everything but the outer `mouse-edge-band-frac` = 30% of each half-axis) produces **no turn** - pure 1:1 aim - then an **ease-in** ramp (the fraction into the band, squared - gentle at the band lip) reaches `max-rate` at the far edge cell, signed by which side of centre `r` is on. Returns 0 in the centre region, for a nil / `<= 1` axis. It is the mirror of the band `reticle-axis` clamps into: the reticle reaches the panning band only by being pushed against a border.
- **Rate constants** (per frame at sensitivity 1.0): `mouse-yaw-rate-max` 0.16 rad/frame at the far left/right edge (a full 360 in ~0.65s held there), `mouse-pitch-rate-max` 0.06 pitch-fraction/frame at the top/bottom edge. Pitch is the clamped [-1, 1] fraction via `state/clamp-pitch`, so holding the reticle at the top ramps the view fully up (then saturates).
- **Reticle right of centre -> +yaw** (pan right), **left -> -yaw**. **Reticle above centre -> +pitch** (look up): rows grow DOWNWARD, so the top of the screen has a *smaller* row, hence the rate is **negated**.
- **Held at an edge keeps panning.** The terminal sends reports only on **motion**, so a pointer held still off-screen yields **no new bytes**. The edge-pan reads the **cached** `:pos` every frame, so an empty `keys` string still returns the pan from the last known position - the camera keeps turning while the pointer rests at (or past) an edge, so the player can aim beyond the current view. This is the key movable-model property: a pointer parked or left at an edge keeps the camera panning with zero new input.
- **Pointer-drift proof.** Reticle in the centre region = pure 1:1 aim, no camera turn; push it to a border = pin there + pan; bring it back = stop. When the OS pointer wanders out of the window the reticle stays pinned on the border (it cannot follow past the clamp) and edge-pan keeps the camera turning - **recoverable**, never the mid-screen freeze of raw mouselook. Re-entry just re-tracks from wherever the pointer reappears.
- `dims` is the live `[rows cols]` (the `& [dims]` arg); it supplies the axis lengths the reticle clamp + band need. Omitting it (used by any non-loop caller) yields a nil reticle and zero yaw/pitch while fire intent still parses, so nothing auto-aims without knowing the viewport.
- `:pos` threads forward as the next frame's cached pointer (in `game-loop`, alongside `prev-keys`). `game-loop` seeds it to `nil` at game start and resets it to `nil` on a **resize** - a nil cached pointer gives `nil` `:aim-col`/`:aim-row`, so the reticle re-centres and comes back nil until the next report instead of pinning a coordinate-shifted cell (no jolt).

`commands/play` stamps the reticle cell on the world (`state/set-reticle`) while the mouse is on and not paused, and resets it to centre (`state/reset-reticle`, the nil sentinel) when the mouse is off, paused, or on a resize; `apply-mouse-look` folds the edge-pan yaw/pitch into the player before the tick (so the frame renders + aims with the new heading), gated on not-paused so the camera can't drift while a menu is up. The world also carries `:scene-cols` (the cast width) so the off-centre hitscan can map the reticle column to a ray (see [Off-centre firing](#off-centre-firing-the-reticle-column-aims-the-shot)).

The reticle math (`reticle-axis`, `edge-pan-rate`) stays **pure** in `glue/controls` and is unit-tested (1:1 tracking, edge clamp / pin, centre-region vs band, ease-in shape, sign, sensitivity scaling on the edge-pan, held-at-edge keeps panning, return-to-centre stops, nil/degenerate axis). The escape re-assert that re-arms reporting lives in `io/` (next section).
...
### Aim point: the movable crosshair (hidden OS pointer)

The aim is the **`+` crosshair**, which follows the mouse 1:1 (`paint-crosshair` draws it at the world's reticle cell `:aim-col` / `:aim-row`, or screen centre when the reticle is nil / mouse off). The OS pointer itself stays hidden - it is *not* the aim indicator (the terminal offers no pointer warp, and a visible arrow racing the `+` would only confuse). Three things keep that contract:
...
- **OS pointer hidden (issue #295).** The OS mouse pointer (the arrow the terminal draws) is a different cursor from the text caret, so `\e[?25l` never affected it - and enabling any-motion tracking makes xterm's default (XTSMPOINTER 1 = hide only when tracking is off) actually *show* it. `mouse-enable` now also sends XTSMPOINTER `\e[>3p` (always hide the pointer, even leaving / entering the window); `mouse-disable` restores the default with `\e[>1p`. Terminals that do not implement XTSMPOINTER ignore it (the pointer can stay visible there), and since no escape can warp or confine the pointer, this hides but cannot truly *lock* it inside the window - the clamp-to-viewport reticle + edge-pan above is what keeps a drifting pointer from ever freezing the aim. On terminals known unable to hide the pointer at all, the startup notice (see [per-terminal reality](#hiding-the-os-mouse-pointer-per-terminal-reality-issue-312) below) tells the player the visible arrow is cosmetic.
- **Crosshair is always the aim indicator while mouse-aiming.** `paint-crosshair` reads a `:mouse` flag (the Mouse setting, threaded through `frame-stats`). When the mouse is ON, an `:off` Crosshair style still draws a minimal dot `·` so the player is never left with no aim point. With the mouse OFF, `:off` keeps hiding the idle reticle exactly as before. The hit-marker (`✗` kill / `×` wound) overrides regardless of style or mouse state, at the reticle position.

### Off-centre firing: the reticle column aims the shot

Because the reticle is no longer pinned to screen centre, the **hitscan aims at the reticle column**, not the centre axis. `core/combat.aim-angle-offset(aim-col, scene-cols)` maps the reticle's screen column to the per-column FOV ray the renderer casts for it - `atan((aim-col - center) / fov-proj-dist(scene-cols))`, sharing the engine's `fov-proj-dist` so the shot's ray is identical to the column the reticle sits on. Every fire path (`shoot` / `pierce` / `spread-shoot` / `beam-impact`), the wall probe, and shot-direction knockback fire along `:angle + offset`, so the enemy whose projected sprite the `+` covers is the one struck; multi-target patterns (pistol pierce, shotgun cone, BFG splash) centre on the reticle column too. A nil `:aim-col` (mouse off) offsets 0, so the shot fires dead ahead and the whole path is byte-identical to the pre-#324 centre aim. The reticle **row** stays informational: enemies are full-height billboards, so the column alone selects the target (the existing vertical gate still uses screen centre, see [combat.md](combat.md)).
