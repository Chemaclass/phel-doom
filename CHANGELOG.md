...
- Crosshair hit-marker: a connecting shot flashes over the reticle for ~0.12s - bold-red `✗` on a kill, bold-yellow `×` on a wound. New `:hit-fx {:kill? :ttl}` field stamped in the combat step (single-target, pierce/spread, BFG-splash); fires only on confirmed contact, kept under reduced motion.
- Reduced-motion toggle (Settings, default off; `PHEL_DOOM_REDUCED_MOTION=1` to force on): photosensitivity-safe mode gating strobing beats - drops lights-flicker scanlines; holds the heartbeat edge, berserk border, pulsing HUD labels and powerup banners steady; suppresses the jump-scare flash; static door-eye. Essential single-shot feedback (hit-vignette, kill flash) is kept. New `:reduced-motion` setting.
- High-contrast HUD toggle (Settings, default off): un-dims grey HUD elements - compass non-facing letters, empty heart pips, healthy ammo-reserve count render bold white instead of SGR-dim/grey - so the HUD reads on washed-out terminals. New `:high-contrast` setting.
- Two new pause/options settings: **Crosshair** style (`+` / `·` / `○` / `off`, hit-marker still flashes when off) and an always-on **Run timer** that appends elapsed time (`M:SS`) to the HUD. Both persist to the settings JSON; the `:enum` cycler was generalised so each field carries its own `:choices`.
- Pickups now render as real Freedoom item sprites instead of flat coloured quads: the LOD threshold dropped to `max(2, 2*lodr)` with billboard height `min(16*lodr, 0.55*sprite_h)`, so sprites engage at any distance where the item is >= 2 rows tall. Coloured-glow fallback kept for extreme range / `PHEL_DOOM_NO_SPRITES=1`. Pickup sprites bake without the enemy gray-ramp-shift via a separate `rgb->256-pickup` (gamma=0.50 lift only).
...
- Distance fog fades toward a near-neutral light-grey haze instead of pure black, with a filmic (ACES-ish) brightness curve: distant walls and floor converge on the same grey haze (aerial perspective) and mid-tones gain contrast, so depth reads "hazy/far" not just "dark". Baked at load (`fade-256-fog` lerps each code in RGB and re-quantizes via `palette/nearest-256`), so the hot path is unchanged. `PHEL_DOOM_FLAT_FOG=1` restores fade-to-black.
