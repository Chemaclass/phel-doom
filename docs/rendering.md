...
**Baked palette:** `enemy_sprites_data.phel` stores native-resolution xterm-256 grids produced by `tools/bake-enemy-sprites.phel`. The bake tool maps Freedoom PLAYPAL entries through two stages before quantizing to xterm-256: (1) per-channel gamma=0.50 (square-root) lift via `tone-lift` - this pushes dark-brown shadow bands (source lum ~15-40) into distinguishable code territory while preserving hue, since all three channels scale equally; (2) a gray-ramp shift of +3 steps applied post-quantization via `gray-shift` - grayscale ramp codes (232-255) are shifted upward by 30 lum, separating the main body shadow tone (lum~58 -> lum~88) from wall grays without affecting the coloured cube codes (warm browns, reds, yellows stay at their baked values). The `rgb->256` base function finds the nearest xterm-256 code by squared-distance against actual cube levels (0, 95, 135, 175, 215, 255) and the 24-step grayscale ramp. An earlier version assumed evenly-spaced levels (0, 51, 102, ...) which systematically mis-quantized brown/tan source colors into gray-green codes, producing color-confetti. The baked data was corrected in-place; re-baking from a WAD applies both fixes via `rgb->256-lifted`.
