...
The showcase reuses the real engine and the real play loop end to end. It
adds one thing: a pure per-phase world transform applied right after
...
minimap on, so the 2D top-down view sits alongside the 3D one. It reuses
existing game features only: `:show-map` (the M-key minimap toggle) plus
`:full-map?` (the `--full-map` no-fog reveal). The demo adds no render
path of its own. It flips flags the game already honours.
...
The interior cover walls ("brinks") are the LAST reveal by design. Phases
1-3 flatten the level to an open arena, and only phase 4 keeps the real
generated maze.
...
- **arena** (phases 1-3): `flatten-interior` rewrites the grid. The outer
...
- **weapon** (off in phase 1): sets the generic `:hide-weapon?` flag.
  `combat/tick-shooting` then swallows the trigger (no shot, no dry-fire click)
  and the render layer skips the gun sprite. Core knows nothing of the demo. It
  honours "no weapon in hand".
...
It also stamps `:demo-phase`, a diagnostic marker nothing branches on.

## Wiring

- `src/demo/phases.phel` - the pure phase data + transform (unit-tested in
  `tests/demo/phases-test.phel`). All demo-specific logic lives here, so the
  showcase stays out of the game code.
- `src/commands/demo.phel` - the thin CLI command. Parses `--phase`, calls
  `run-demo-session`.
