...
- DOOM-style vertical levels (issues #231-#237, #240, #243): the raycaster gained variable terrain. Cells carry per-cell floor heights (#232, a shaded riser plus a flat cap) and ceiling heights (#235, hanging faces for low tunnels or taller atria); Z physics (#233) makes the eye follow the floor underfoot, auto-climb steps up to 0.4, block taller risers like a wall, and ease down ledges and pits under gravity; enemies anchor to their cell floor (#234) instead of floating at the horizon; the ↑ / ↓ arrow keys pitch the view up / down (#231, #240) as a pure horizon shear (no extra rays); and hitscan is vertical-aware (#243), so a shot must land on the enemy's drawn sprite, not just its column. Levels opt in via `:floor-heights` / `:ceil-heights`; with everything flat the frame is byte-for-byte identical to before.
- Walkable verticality in the campaign (issues #237, #270): Level 2 is a hand-authored stair / platform / pit showcase, and Levels 3, 5 and 7 each gain a distinct feature - L3 a descending trench, L5 a raised dais in the cyberdemon arena, L7 a multi-tier stair room (see #298). Every climb is within the auto-climb limit and the exit door + keycard stay reachable, so difficulty and progression are unchanged. Level 1 stays flat (the golden-render fixture); the run is still 10 levels with the boss arena last.
- Level 7 is now a spacious three-tier stair room (issue #298): the old single-ledge revenant room is reworked into one open 42x26 arena split into three stacked floor tiers (z 0.0 / 0.75 / 1.5) by two divider walls, each pierced by two stair mouths. Four three-cell-wide staircases with OPEN sides (you can fall off a run, #299) connect the tiers, and each tier is reachable ONLY by walking up a flight. Every step is a 0.375 rise (below the 0.4 auto-climb limit, and exactly representable so a run never silently blocks on rounding drift), so the stairs are walkable up and down to every tier. The yellow-locked exit sits on the bottom tier (reachable with zero climbs), and L7 keeps its identity (revenant + demon + baron mix, chase 1.8, yellow lock, BFG drop). Edge physics (handrails + fall damage off the open lips) land in the same release (#299, below).
- Color-coded floor tiers (issue #304): raised floor tiers are now tinted by height so a multi-tier room (L7) reads as distinct levels at a glance instead of one flat field of grey stone. The lower raised band reads cool steel-blue, the higher band reads magenta-rose; the ground (and every flat level) stays neutral grey. The tint preserves the distance fog and side-shading (a far tier still fades into haze) and is a strict no-op on flat floors, so flat frames - including the golden render fixture - are byte-identical. Rose was picked over a warm amber so the tier stands out against the warm-grey stone rather than blending into it. The riser faces and step caps both carry the tint so the staircase + platforms pop.
- Dark platforms, orange stairs (issue #322): multi-tier rooms now read at a glance instead of as floating coloured slabs. Two changes: (1) the raycaster tracks the nearest TWO floor risers per ray instead of one, so a stair run draws as a stacked pair of ascending treads rather than a single step plus flat ground beyond (each L7 / L6 run is exactly two steps between tiers, so the second riser captures the rest of the flight you can see); (2) the palette was reworked by ROLE - the flat platform surfaces (tier floors + step tops) are now DARK near-neutral stone (the higher tier darker than the lower), and the vertical stair RISERS are ORANGE, so the stairs pop as a clearly climbable ramp against the quiet dark rock (replacing the rejected pink / blue tint). Each step still carries a lit-to-shadow gradient and a dark nose edge so the treads read as distinct. The cast stays allocation-equivalent (one extra accumulator in the same DDA loop, a fixed handful of extra arrays, no per-column churn) and all the new work is dead on flat levels, so every flat frame - including the golden render fixture - is byte-for-byte identical. Measured cost is contained to the multi-tier rooms (a few percent on the stair scene, zero elsewhere).
- Cross-tier enemy occlusion (issue #302): enemy sprites now clip against the tier geometry in front of them, not just the full-height walls. An enemy on a higher tier shows over a platform edge when the line of sight is clear, and one standing behind a nearer floor riser (or under a nearer hanging ceiling) has the blocked part of its body hidden - or vanishes entirely when fully behind the step - instead of drawing through it. The clip is a pure span-intersection helper (`clip-sprite-span`) wired into the sprite draw: a riser nearer than the enemy clamps its feet to the riser's on-screen top edge, a nearer ceiling drop clamps its head to the drop's lower lip. Single nearest tier per column (px1); flat levels carry no tier bands so the clip is a no-op and the frame stays byte-for-byte identical (golden hashes unchanged). Multi-tier stacking, pixel-doubled parity, and the shadow / face / pickup overlays are noted follow-ups.
...
- Tier handrails and fall damage (issue #299): the open-sided tiers and stairs of the L7 room now have real edge physics. A tier or stair edge can carry a handrail, a low barrier you see over but can't pass, so you can't step off that downward edge (it blocks like a wall). Where there is no rail, walking off an edge with a drop bigger than the 0.4 auto-climb limit is a fall to the tier below that costs 1 life per tier dropped (a two-tier plunge costs 2); a deliberate stair descent (each tread is a small 0.375 step) is never a fall and costs nothing. Both the player and enemies obey the same rule: an enemy that walks off an unrailed lip falls and takes the same damage, and can die from the drop. L7 rails the west top-plateau lips (so you can't tumble off there) and leaves the east lips open (a real fall), with all four staircases fully walkable and the exit + key still reachable. Fall damage is environmental (armor doesn't absorb it, no i-frames). Flat levels carry no rails and never trigger a fall, so they stay byte-for-byte identical (the golden render fixture is unchanged).

### Changed

- Movable aim crosshair (issue #324): mouselook is reworked from positional camera steering (#318/#320, which felt like a joystick) into a movable `+` reticle. The crosshair now follows the mouse 1:1 (clamped to the 3D viewport) and the gun fires toward wherever it points - an off-centre hitscan that strikes the enemy whose projected sprite the reticle covers, not the fixed screen-centre axis. Pushing the reticle into the outer edge band pans the camera (yaw left/right, pitch up/down) so you can aim beyond the current view; the central region is pure 1:1 aim with no camera turn. Because a terminal cannot lock, warp, or hide the OS pointer (iTerm2 included, #313), the reticle is the pointer's ABSOLUTE position clamped to the viewport: when the pointer wanders off the window the reticle pins at the border and edge-pan keeps turning, so aiming never freezes mid-screen the way raw 1:1 mouselook would. The Mouse on/off toggle, the Sensitivity slider (which now scales both the reticle speed and the edge-pan rate), and click / drag to shoot are unchanged; with the mouse off the reticle pins to centre and the game plays byte-for-byte like the keyboard-only build (the golden render fixture is unchanged). The reticle row is informational for now - enemies are full-height billboards, so the column selects the target.
...
- Enemies cannot levitate up tier risers (issue #301, part 1): the chase step is now floor-height-gated, mirroring the player's own step-up rule. An enemy can no longer 2D-walk UP a stair flank or tier riser taller than the 0.4 auto-climb limit; tall risers block like a wall while descents still pass, so monsters walk down off ledges but can't float between tiers (visible on the L7 stair room). The chase slides along a tall flank and takes the open, climbable stair mouth instead of vaulting the side. Flat levels seed an all-0.0 floor grid so the gate is inert and the frame stays byte-for-byte identical. Stair-seeking navigation (actively routing toward a stair) is a separate follow-up.
- Enemies route to and use the stairs (issue #301, part 2): completes the riser work. When a chasing enemy is fully boxed in by risers (every coarse step is a wall or a too-tall climb), it now sweeps a finer angle fan and steps onto the stair-mouth tread that heads toward the player's tier instead of freezing against the divider wall. So an imp on the top tier of the L7 stair room walks DOWN both flights to reach a player at the foot of the staircase, and one below climbs up - they actively use the stairs, not just respect their edges. The extra search is greedy (it follows a mouth that lies roughly along the chase bearing; an off-bearing maze switchback would need a path planner, left as a follow-up), costs a fixed 8 probes, and runs only on the frames an enemy is actually riser-blocked. Flat levels never reach the new path, so the frame stays byte-for-byte identical.
- Enemies navigate to stairs from any position (issue #311): the greedy stair-seek (#301 part 2) only finds a stair mouth roughly along the chase bearing, so an enemy off to the side of the mouth on a different tier than the player wedged against the divider wall and never reached them - cross-tier combat broke whenever the only way up or down was a stair the monster was not facing. A chasing enemy that cannot make greedy progress toward a player on another tier now runs a bounded breadth-first search over the grid (walls block; a step may rise or drop at most the 0.4 auto-climb limit, so the route threads the smooth stair treads and never plans a suicide drop off a ledge) and takes the first step along the shortest path to the player, routing to and through any stair mouth from any position. The search is bounded (it stops at the player cell, or after sweeping the room) and cached on the enemy (recomputed about every 1.2s, on reaching the cached waypoint, or on a tier change), so it stays off the hot path. Greedy still owns every same-tier chase and every flat level, where the BFS never runs, so flat / same-tier behaviour is byte-for-byte identical (the golden render fixture is unchanged).
- Level 6 is now a spacious three-tier stair room (issue #310): the spectre level gets the same multi-tier treatment as L7, rotated onto the east-west axis so the campaign reads as a varied multi-level space instead of one showcase. The old procgen room becomes one open 42x26 arena split into three stacked tiers (z 0.0 / 0.75 / 1.5) by two solid divider columns, each pierced by two open-sided staircases, so the player climbs four runs west from the bottom (east) spawn tier up to the middle then top tiers. Every step is a 0.375 rise (below the 0.4 auto-climb limit, exactly representable so a run never silently blocks on rounding drift), the stairs are walkable up and down to every tier, and a tier is reachable only by walking up a flight. The unlocked exit stays on the open bottom tier (reachable with zero climbs), tiers color-code by height automatically, and enemies use the stairs + clip across tiers for free. L6 keeps its identity (spectre + imp + caco mix, chase 1.7, no lock).
- Level 8 is now a grand three-tier archvile court (issue #310): the archvile court gets the same multi-tier treatment in the largest footprint (50x30) - a wide southern court pit (the spawn floor) climbing north up two galleries to a throne band. The old procgen room becomes one open arena split into three stacked tiers (z 0.0 / 0.75 / 1.5) by two solid divider rows, each pierced by two open-sided staircases. Unlike L7's roughly-aligned runs, the lower and upper stairs sit at different columns, so the ascent to the throne crosses the gallery (a third distinct multi-tier shape alongside L6's east-west room and L7's aligned north-south one). Every step is a 0.375 rise (below the auto-climb limit), the stairs are walkable up and down to every tier, and a tier is reachable only by walking up a flight. The unlocked exit stays on the open court (bottom) tier (reachable with zero climbs), and L8 keeps its identity (archvile + caco + mancubus mix, chase 1.9, no lock).
- Level 4 is now a three-tier baron stronghold (issue #310): the blue-locked baron level becomes a 44x28 stronghold and the only LOCKED multi-tier room - a wide southern hall (the spawn floor) climbing north up to two raised baron heights via two stair spines on the outer flanks (twin west + east spines hugging the side walls), so the barons hold the wide central heights while the player funnels up the edges (a fourth distinct shape alongside L6's east-west room, L7's centre-aligned north-south one and L8's offset court). Three stacked tiers (z 0.0 / 0.75 / 1.5) split by two solid divider rows, each pierced by two open-sided staircases; every step is a 0.375 rise and a tier is reachable only by walking up a flight. The blue-locked exit stays on the open bottom tier (reachable with zero climbs), and because every floor cell is reachable the randomly-placed blue keycard is always fetchable wherever it lands (a key may sit up a stair, the exit never can). L4 keeps its identity (baron + demon mix, chase 1.4, blue lock, chainsaw drop).
...
- Vertical hitscan accounts for an enemy's floor height (issue #291): the vertical aim gate compared the crosshair against a sprite drawn centred on the horizon, but the renderer lifts a sprite on a raised cell UP the screen by its floor-offset. On the raised cells the verticality pass added (L5 dais, L7 ledge) a level shot then hit empty floor below a lifted enemy and rejected a shot aimed at the drawn body. The gate now subtracts the same per-target floor-offset (read from the world's `:floor-pgrid` at the enemy's cell) so it agrees with the pixels drawn. `floor-offset-rows` moved into `core/projection` so the renderer and the hit gate share one definition. Flat floors give a zero offset, so flat-floor and pitch-0 behaviour is byte-identical and the golden render hashes are unchanged.
...
- Enemies no longer float above the ground (issue #297): the billboard was centred on the horizon (`(svh - h)/2 + pr - floor-off`), so the feet hung a row above the projected floor at most distances. Sprites are now anchored by their FEET on the projected floor via a new pure `sprite-feet-row` and drawn upward by their pixel height, so the feet land exactly where the floor texture meets the wall at every distance. The grounding shadow, face glyph and floating HP digit ride the same anchor; a monster on a raised cell (L5 dais, L7 ledge) stands ON its platform because the lift is folded into the projection (no double-counted floor-offset, the v1 bug). The vertical hitscan gate reuses the SAME `sprite-feet-row`, so a shot lands on the drawn body and one aimed at the bare floor below or sky above misses, with the FIXED screen-centre crosshair. The golden render hashes are unchanged (the golden fixture has no enemy on-screen).
- Multi-tier rooms read as stepped structure instead of floating enemies on flat blue slabs (issue #319): in the L7 / L6 stair rooms an enemy on a raised tier had its feet correctly lifted but no platform drawn under it (the renderer paints only the nearest riser and a flat z=0 floor), and a near step drew as one flat tinted block. Two render fixes: (1) a tier-tinted ground PLATFORM PAD is now painted under any enemy on a raised cell (a bright lit lip on the feet row over darker face rows), so the billboard visibly stands on tier stone in both sprite and glyph modes; (2) the riser is now lit like a real staircase - the tread cap brightens, the vertical face drops into shadow, and a near-black NOSE line traces the tread's front edge - so the step reads 3D instead of a flat slab. Both are drawn only on raised cells (`fz>0`), so every flat level is byte-identical and the golden render hashes are unchanged. Drawing every tread of a multi-step run at its true height (the cast currently tracks only the nearest riser) is a separate follow-up.
