...
- **Mouse look (#246, #324)**: the `+` crosshair stays fixed at screen centre (the gun fires dead ahead; left-click to fire, held = auto-fire), and pushing the mouse pointer into a screen edge pans the camera so you can turn and aim beyond the current view. On by default, toggled by the **Mouse** setting; edge-pan speed scales with **Sensitivity** (50% = neutral 1.0x). Purely additive - the keyboard path is untouched. See [input.md](input.md#mouse-look-issue-246).
