...
- `step-bands`: the per-column list of the FARTHER step faces (every riser beyond the nearest), stride 3 `[top bot dist ...]` near-to-far. Absent for a single-step column. `:spans` for a column are contiguous, so this is built with a fetch-mutate-store (PHP copies nested arrays by value; the store writes the mutated copy back). One pure helper reads it in the row loop: `farther-face-vstep` returns a row's depth into whichever farther face it lands in (0 at the lit top, growing toward the shadow base - the `tier-riser-ramp` index), or -1 for a tread / platform row.

The row loop (both the px1 half-block path and the px2 pixel-doubled path) tests, in order: sky, then the nearest step face `[riser-top, riser-bot)` shaded by `tier-riser-ramp[min(row - riser-top, 5)]` (row 0 = the lit lip); then the plateau above `riser-top`, where each row is either a farther step's shaded face (`farther-face-vstep >= 0`, indexing the same riser ramp) or the receding tier-top surface between the steps; then it falls through to the ordinary floor/wall logic unchanged. On a flat world `:spans` is empty, so every column's `riser-top` aget reads `nil`, `has-riser?` is false, `step-bands` is absent, and all the new branches short-circuit as dead comparisons (see "Perf" below).

**Riser face** shades top-lit to shadow-base down its height via `tier-riser-ramp` (a 6-step slate gradient, `48;2;96;104;132` lit -> `48;2;26;29;42` shadow). The emitters index it by the row's depth into the face: `min(row - face-top, 5)`. Index 0 - the top row of every rise-face - is the lit leading edge (the "step lip"), which keeps individual stairs countable when the treads between them collapse to sub-row at a low viewing angle. The gradient gives the vertical face volume (it read as a 2D sticker when flat), while the distinct slate hue stops it blending into the walls or floor. This deliberately avoids the #373 `riser-shade-level` approach (sampled the wall shade table -> risers read as walls); the shading is intrinsic to the face's own height, not a wall tint. For the nearest step the emitters read `riser-top`/`riser-bot` directly; for farther steps `farther-face-vstep` returns the same depth index off `step-bands`.

**Tier-top floor** is the raised-tier ground: the treads between steps and the platform capping the staircase, visible between the far wall's bottom and the nearest riser's top. It recedes via `tier-top-ramp` (a 4-step slate gradient, `48;2;72;79;104` near -> `48;2;41;45;62` far), indexed by height above the riser so the surface fogs brighter-near to darker-far - a receding plane, not a flat bar. Same slate hue, one notch lighter than the riser face, so a raised tier + its stair edges read at a glance. Earlier drafts sampled the floor stone texture here (issue #384, via `tier-top-row-dist` + a per-column fog level) so a tier looked like textured ground continuous with the floor; that was reverted because a tier that matches the floor is exactly what made platforms and stairs hard to parse. The gradient keeps that legibility (distinct hue) but adds the depth the flat fill lacked. Both ramps are module constants indexed by a per-cell `aget` (no per-column colour storage, no closure). `src/core/projection.phel`'s `tier-top-row-dist` remains (unit-tested, and the basis for the ceiling mirror `ceiling-top-row-dist`) but is no longer on the render path.
