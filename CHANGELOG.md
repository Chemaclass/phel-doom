...
- New **Texture filter** setting (#462, off by default): far walls sample a pre-filtered smaller copy of the stone texture instead of point-sampling the full 64x64, which is what makes distant walls sparkle as you move. Measured on the L1 fixture, the wall band's colour churn drops from 82.5% to 79.9% and the emitted frame shrinks ~1.2%; frame time is unchanged either way. Off is byte-identical to before. The floor half is still open - it is 3x the cells and where most of the speckle lives.
