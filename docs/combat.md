...
The crosshair is fixed at screen-centre (`svh/2`). An enemy at distance `d` projects to a sprite of half-height `projection/sprite-half-rows(svh, d, scale)` rows, centred on `svh/2 + pr - foff`. `pr = projection/pitch-rows((:pitch player), svh)` is the pitch shear in scene rows, and `foff = projection/floor-offset-rows(proj-dist, floor-z, d)` is the projection of the enemy's CELL floor height, the same lift the renderer applies to stand the sprite on a raised platform (issue #291). The crosshair lands on the sprite iff `|pr - foff| <= sprite-half-rows`. The gate (`enemy/vertical-hit?`) wraps the existing perp test in `shoot`, `pierce`, and `spread-shoot` (primary + graze). `sprite-half-rows` mirrors the renderer's billboard height (`min(svh, scale * proj-dist / (max(0.5, d) * char-aspect))`, halved), so the check is screen-space exact: closer enemies (taller sprites) are more forgiving, distant ones demand near-level aim, and the cyberdemon's 2x sprite (`scale`) is correspondingly easier to keep on.

The per-target `floor-z` is read from the world's `:floor-pgrid` (the float[][] floor-height twin, #232) at the enemy's cell `(floor y, floor x)`, threaded through `shoot` / `pierce` / `spread-shoot` as a trailing `floor-pgrid` argument. Before #291 the gate had no `foff` term, so on the raised cells the verticality pass added (L5 dais `fz 0.8`, L7 ledge `fz 1.05`) a level shot hit empty floor BELOW a lifted enemy and rejected a shot aimed at the drawn sprite (foff 3-11 rows vs half-rows 2.5-5.5, a gross mismatch). `floor-offset-rows` lives in `core/projection` (alongside `wall-px`) so the renderer's sprite anchoring and this gate share ONE definition; `render/frame-math` re-exports it for its existing callers.

`svh` (scene rows) is supplied by the game loop, which stamps `:scene-rows` on the world from `render/scene-rows` (the same `rows - hud-rows` height the renderer draws). Combat stays pure: `svh`/`pr` are plain ints in, `floor-pgrid` is plain data. A world without `:scene-rows` (headless / legacy tests) disables the gate, which is identical to level aim on flat ground (`pr = 0`, `foff = 0`, `|0| <= half-rows` always), so flat-floor pitch-0 behaviour is byte-for-byte unchanged. The BFG path (`bfg-fire`) is splash-around-an-impact, not a crosshair hitscan, so it is not gated.
