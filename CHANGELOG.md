...
- Truecolor fog tint decoupled from the 256-neutral constraint (#419): in truecolor mode the distance fog now fades toward a genuinely cool haze (rgb 104/110/130) instead of the near-neutral grey the 256 palette is forced into. Same luminance, real cool bias, no blue band. The 256 path is unchanged; only `:truecolor` / `PHEL_DOOM_TRUECOLOR` renders show it.
- Room lighting (#418): opt-in **Room light** setting (off by default) folds a per-cell light bias into the wall shade so levels read as dark rooms with lit pools instead of even distance shading. The bias grid is derived from the map at load (lamp pools on a coarse lattice, `core/light`) and looked up one-per-column at the ray's hit cell, so the hot path stays within budget (+0.5-2.5% render). Off is byte-identical; derived like `:pgrid` (rebuilt on grid change / load, never saved).

### Changed

- Frame cap raised to ~120fps (was ~60fps): `standard-frame-us` 16667 -> 8333, since measured hardware renders 97-222fps uncapped. Input feel is unchanged - every per-frame constant was first converted to a frame-rate-independent unit: the movement/turn/pitch/sprint hold counters count down in seconds (not decrement-by-1), and the mouselook edge-pan rate (`mouse-yaw-rate-max` / `mouse-pitch-rate-max`) is per-second scaled by `dt`. Resize-poll and mouse-re-arm cadences double (`resize-poll-frames` 6->12, `rearm-mouse-frames` 60->120) to hold the same wall-clock timing. Render byte-identical. Savegames no longer persist the `:moves` hold counters and always load at rest, so a mid-walk save loads standing still and old int-frame-count saves can't be misread as seconds of auto-walk.
- Mouse turn is faster + survives the pointer leaving the window (#324): `mouse-look-gain` 3.0 -> 5.0 so one swipe covers most of a turn (a terminal maps the pointer to a coarse ~80-180-column grid). The edge-pan is now **ungated** - it keeps turning every frame the pointer sits in the outer border band (read from the cached position), so a pointer pinned at or past a window edge keeps swinging instead of freezing, since no terminal can lock/hide the OS pointer (#313). A resting mid-screen pointer never drifts. Ghostty joins the startup cosmetic-arrow notice. Keyboard-only play unchanged.
- Upgraded phel-lang 0.46 -> 0.48. Drop-in (all tests green): the codebase already builds at `withOptimizationLevel(2)` and tags its hot-loop locals (`^int` / `^float`), so it automatically picks up 0.48's newer compiler lowerings - tag-typed native math/string ops, `(** x 2)` -> native `x*x`, `reduce` over a tagged vector compiled to a native `foreach`, and tail-position `foreach`/`doseq` emitted without an IIFE closure wrapper. Also clears the PHP 8.3+ `ReflectionType` deprecation warnings.

### Performance

- Render memo/dedup pass: `frame-gradients` also caches the sub-row sky codes + four floor-cast tables (rebuild only on resize/pitch/pixel-doubling toggle); `emit-scene-px1` hoists `vw-1` and reads a precomputed per-column wall-height instead of recomputing `max(1, bot-top)` per row; `paint-face-overlay` / `paint-enemy-hp-flashes` reuse the zone pass's `enemy-projs` instead of re-projecting. Byte-identical; wins land on resize/pitch/combat frames the fixed-viewport bench doesn't exercise.
...
- Enemy counts rebalanced by room size and threat weight (HP x per-hit damage): L5 no longer stacks five boss-tier cyberdemons (now 2 + imp fodder) that out-gunned the L10 boss; sparse large halls (L4, L6, L9) populated so difficulty ramps smoothly. L1 tutorial and L10 boss untouched.
- L2 demon room is no longer a bare box: scatters random interior wall pillars (`:walls`) like every other room, for cover + per-run variety. Pocket-sealed into one connected floor region with the single exit on a random reachable wall, so no run dead-ends.
