...
The row loop (both the px1 half-block path and the px2 pixel-doubled path) tests, in order: sky, then the nearest step face `[riser-top, riser-bot)` - its top row (`row == riser-top`) is the lit lip (`tier-riser-ramp` index 0), every row below samples the slate-tinted wall texture (see **Riser face**); then the plateau above `riser-top`, where each row is either a farther step's face (`farther-face-band >= 0`, its own lip + texture) or the receding tier-top surface between the steps; then it falls through to the ordinary floor/wall logic unchanged. On a flat world `:spans` is empty, so every column's `riser-top` aget reads `nil`, `has-riser?` is false, `step-bands` is absent, and all the new branches short-circuit as dead comparisons (see "Perf" below).

**Riser face** is textured stone (#404): every row below the lip samples `wall-tex-px` at `texRow = min(63, (row - face-top) * 64 / face-height)`, column `u`, and maps the texel through `tier-riser-tex` - a `[level][texel]` table (24×256, same shape as `tex-fade-table`) that recolours each grey wall texel into the dark-slate family (`luma -> (luma*0.55+20, *0.6+24, *0.75+34)`, lerped toward the `tier-riser-ramp` shadow-base `26,29,42` by `level/23` so distance haze still applies but never leaves the slate). This keeps the texture's PATTERN (a face shows material, not a flat sticker) but in a distinct dark-slate hue, so a riser never blends into the grey floor - the failure mode of the first cut (#404 iter 1) which sampled the plain grey texture and read as literally the floor (the same #384 problem, moved to the face). The top row of every face (`row == face-top`) stays the flat bright `tier-riser-ramp[0]` lip (`48;2;96;104;132`), the countable-stairs edge that reads even when a face is 1-2 rows tall. This supersedes the #402 flat slate gradient (a shaded flat colour still read as a sticker at riser height) and deliberately avoids the #373 approach (sampled the wall SHADE table, not the wall TEXTURE, so it read as a flat wall tint). The nearest face reads `riser-u`/`riser-level`; farther faces read their own from `step-bands`.

**Tier-top floor** is the raised-tier ground: the treads between steps and the platform capping the staircase, visible between the far wall's bottom and the nearest riser's top. It recedes via `tier-top-ramp` (a 4-step slate gradient, `48;2;72;79;104` near -> `48;2;41;45;62` far), indexed by height above the riser so the surface fogs brighter-near to darker-far - a receding plane, not a flat bar. Same slate family as the riser face, so a raised tier + its stair edges read at a glance. It is deliberately NOT textured: sampling the floor stone here (issue #384, via `tier-top-row-dist` + a per-column fog level) made a tier look like textured ground continuous with the floor, exactly what made platforms hard to parse - the tier-top keeps the distinct flat-hue gradient. `tier-top-ramp` is a module constant indexed by a per-cell `aget`; `tier-riser-tex` likewise (no per-column colour storage, no closure). `src/core/projection.phel`'s `tier-top-row-dist` remains (unit-tested, and the basis for the ceiling mirror `ceiling-top-row-dist`) but is no longer on the render path.
