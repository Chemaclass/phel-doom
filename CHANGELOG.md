...
- New **Texture filter** setting (#462, off by default): far walls sample a pre-filtered smaller copy of the stone texture instead of point-sampling the full 64x64, which is what makes distant walls sparkle as you move. Walls and floor both. Measured on the L1 fixture, the wall band's colour churn drops from 82.5% to 79.1% and the floor's horizon band from 81.7% to 80.5%, with the emitted frame 2.1% smaller; frame time is unchanged either way. Off is byte-identical to before. The near floor is deliberately left alone - those rows magnify the texture rather than minify it, so a smaller level would only blur them.
- Monsters have an attack pose (#463). One baked frame per type meant a monster mid wind-up looked exactly like one standing still, so the `!` telegraph from #457 was carrying the whole read by itself; now the sprite itself changes for the 0.3-0.8s before a swing or a bolt. All eight types, mipped like the resting poses so a distant wind-up reads as cleanly. Costs 24 KB of phar and 28 ms of startup. The walk cycle is deliberately left out: a two-frame alternation at terminal resolution risks reading as silhouette flicker at mid range, which is the shimmer the sprite mips exist to kill.
- The settings page explains itself (#468). Seventeen fields read as one undifferentiated list and nothing on screen said what any of them did. Fields are now grouped under Audio / Video / Play / Controls / Access headers, with a one-line hint for whichever field the cursor is on - both from the same table that drives the page, so a new field cannot ship without them. On a short terminal the list scrolls around the cursor instead of the box growing past the screen.
- New **HUD glyphs** setting (#471). The HUD uses a keycard, a half-heart, an info mark and a bullet; a font without those codepoints draws tofu boxes, which are frequently double width, so they do not merely look wrong - they shift every column after them and break the strip's alignment. `ascii` swaps each for a single-column stand-in, and a non-UTF-8 locale picks it automatically on first run (a saved choice always wins).
- Secret walls have a tell (#469). They looked and cast exactly like ordinary walls, so the only way to find one was to walk the whole level pressing F at every surface - while secrets are worth 30% of the run rank. A secret's stone now runs half a tile out of phase with the walls beside it, which is Doom's own misaligned-texture cue: visible when you look for it, invisible when you do not. Nothing about the cast or the collision changes, and a level without secrets renders byte-identically.
- The run has beats where it used to have cuts (#470). Dying jumped straight to the YOU DIED box, so the blow that ended the run was never seen: the killing frame is now held for 0.6s first (and input during it is dropped, so a panicked keypress cannot pick something on the screen that follows). Clearing a level jumped straight into the next one, so the per-level kills, secrets and time the game already tracked were only ever shown once the whole run was over: a card now reports them, and any key continues. Both are skipped under `--demo` replay.
- The first level opens with a line of key hints (#467). A first-time player landed in L1 with no reminder of the controls: the help panel exists but has to be discovered, and the objective splash says what to do, never how. One dim, steady line along the bottom - move, look, fire, use, help - for fifteen seconds on level 1 only, and it goes early as soon as you have moved, turned and fired.
- The scroll wheel and `[` / `]` cycle weapons (#464). The wheel was parsed and thrown away and there was no next / prev key at all, so switching meant reaching for a number - the one thing every FPS player's hand does mid-fight. Steps through the weapons you own, wrapping both ways, with the same reload lockout and ammo bookkeeping as a slot key; an explicit number still wins in the same frame.
- Monsters and the world have voices, on every OS (#460). Only the seven weapon reports were baked from Freedoom; everything else fell through to per-OS system sounds that exist only on macOS, so on Linux and BSD a hit, a kill, a pickup, a reload and a door were all the terminal BELL - and no monster made any sound at all. 29 more Freedoom sounds are baked in: a per-type wake growl when a monster notices you (the first warning of an ambush, which used to be taking the hit), the fireball leaving a caster, per-type death cries so an imp and a cyberdemon no longer die identically, monster and player pain, and distinct cues for loot, a weapon, a powerup, a door, a refused door and a switch. Volume now reaches `paplay` and `play` as well as `afplay`, so the SFX and Music sliders work off macOS too. Costs 610 KB of phar; cold start is unchanged.
- The crosshair turns red when a shot would connect (#458). Since aiming became vertical (#243) a shot only lands when the crosshair is on the enemy's drawn sprite, so aiming a little at the floor or the sky misses - and the miss read as lag, because nothing confirmed the target before the trigger. The cue asks the real selection for the weapon in hand - the single-target hitscan rule, or the shotgun's cone, which disagree in both directions at shotgun range - so it cannot disagree with the gun about where a shot goes (splash weapons aside: their blast can also catch an enemy the reticle left white). It does not track the trigger: an empty mag or a running reload keeps the target red, since the dry CLICK and the reload prompt already say that. Kill, wound and muzzle-flash colours still take priority.
...
- The attack-windup `!` is warning amber for every monster, the same colour as the low-ammo and reload prompts. It used to take the enemy's own head colour, which comes from the glyph era and has nothing to do with the sprite drawn today - a cacodemon's catalog head is bright cyan while its sprite is red, so the mark read as information rather than a warning, and differently per monster. One colour is one thing to learn.
...
- The help and pause menus no longer paint past the bottom of the screen (#499, #500). The H info panel could drop its CONTROLS and COMPASS sections, but the rest of it was 28 rows - four more than an 80x24 terminal, the default nearly everywhere - and overflow does not clip politely: a terminal clamps every write past its last line onto that line, so the tail of the weapons table and the closing border all landed on the last row on top of each other. Every block is droppable now, added in priority order (the RUN and PLAYER lines, the reason you open it mid-fight, survive longest), and on a short terminal the blank spacer rows go too, which fits the whole panel into 24 rows with the weapons table intact. The pause menu does the same below 14 rows. What each height shows is monotonic - a shorter terminal never shows more of the menu than a taller one - and `centred-box-string` truncates as a last resort, so no menu can paint off screen.
- The enemy face glyph sits on its enemy again in pixel-doubled mode (#477). The overlay projected with the full numerator while the pixel-doubled scene places bodies with half of it, so an enemy one world unit off centre had its face 12 terminal columns away from its body, and one three units off had it 34 columns away - far enough to land on a different enemy or off screen entirely. The overlay also reads the depth buffers at that column, so a face could leak past a wall or vanish behind one that was not there. Only ever visible in glyph mode (sprite mode skips the face overlay), which is why no golden frame caught it.
- The HUD strip no longer paints through the minimap (#466). With the map on at 80 columns, a run carrying a backpack, stamina, a key and a difficulty tag ran the row-2 strip straight through the panel's top border. Optional chips are now appended only while they fit, dropping in reverse priority - run timer, difficulty, keys, backpack, stamina - so a squeeze costs you decoration rather than the resource you spend mid-fight, and the core (level, kills, weapon, ammo) is never trimmed. The help menu also lists sprint and the mouse, and the weapon slots read `1 .. 7`: the docs, the menu and the README all still advertised eight, and combat.md documented a super shotgun that does not exist.
- Movement stops when you let go, on terminals without key-release events (#461). Terminal.app, GNOME Terminal and xterm can only report that a key produced a byte, so every byte re-armed a 300ms hold - and since the OS auto-repeats while a key is down, the last repeat kept the player walking ~300ms after release: past the doorway, into melee range, through the strafe meant to dodge a bolt. The 300ms exists to bridge the OS initial-repeat delay, which is only paid on the first byte; a byte arriving while the slot is still warm is a repeat and now gets 150ms, enough to reach the next one. Kitty-protocol terminals were never affected.
...
- Every frame is cheaper. The fog-of-war scan - the pass that decides which cells the minimap has revealed - ran on every single frame, walking a line of sight out to every cell in range. What it computes depends on which CELL the player is in, not where they are inside it, so standing still or walking across one tile re-derived an identical answer. It now runs only when that changes: a walking frame goes from 1.11 ms to 0.97 ms of state work, a standing one from 1.05 ms to 0.92 ms. Opening a secret or throwing a switch next to you still lights up immediately, and the full revealed map after 400 frames of play is identical.
- Firing is cheaper. Every shot floods the map from the player's cell so enemies in the same room hear it, and that flood cost 1.18 ms a call - for a radius of three cells, at most about 25 of them. The fill was never the problem: its seen-set was a set of coordinate vectors (a persistent-vector hash per membership test), its queue was rebuilt from scratch on every step, and each cell allocated two more vectors. Rewritten on native arrays it takes 93 us, which brings a frame in which you are shooting from 1.49 ms to 1.09 ms of state-transition work before the renderer starts. Who wakes is unchanged, checked over 320 cases.
- Level generation is 3x faster: 42.7 ms per level down to 13.9 ms, and up to 78 ms on the biggest level before. The two passes that dominate it - dropping the exit at a random reachable wall, and sealing any floor pocket the scattered wall blobs cut off - both flood the floor and then scan every cell, and both read the grid through the persistent vector accessor while keying their seen-set by a freshly built `"x:y"` string per neighbour test. Reading through a flat PHP-native mirror, keying by that same flat index, taking the neighbour offsets from an int array rather than a rebuilt vector literal, and writing the wall blobs into the mirror instead of rebuilding a persistent grid per cell takes `place-exit` on a 64x40 grid from 57.9 ms to 8.4 ms and the pocket seal from 58.1 ms to 10.1 ms. Levels are unchanged: the flood consumes no rng and the scan order is the same, so all 210 worlds (10 levels x 7 seeds x 3 difficulties) hash identically.
