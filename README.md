...
- 5 monster types (imps, demons, cacodemons, barons, cyberdemons) — distinct color, body texture, animated face glyph, aggro pulse at close range
- Hitscan combat with blood splatter, muzzle flash, 3–6s respawn cooldown
- 5 lives, i-frame window, blood-red palette flush + 1-frame white impact flash on hit
- Walk-into-door auto-advance, heart pickups (refill life, max 5)
...
the edge, composition in between.
...
Invariants: `core/` never imports `io/` or `glue/`; `io/` may import
`core/`; `glue/` may import both. Anything under `core/` is unit-
testable against bare data — no fakes.
...
keys, edge-detects toggle keys, hands the world off. Render time is
the throttle.

## Performance

`frame->string`: ~2ms (80×24), ~3ms (120×30), ~5ms (180×40). Hot loop
uses direct PHP ops (`php/aget` etc.) instead of Phel polymorphic
dispatch, pre-baked shade tables, run-length encoded rows, PHP-native
nested arrays for the grid, alt screen buffer + cursor-home redraw.

`proj-dist` is decoupled from viewport width — resizing widens the
FOV instead of zooming the walls.
