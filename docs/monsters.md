...
- `spawn-enemies-mixed` - multi-type. Spec shape: `{:type :count [:lives N] [:max-concurrent K]}`. Omit `:lives` and the type's catalog default applies (mixed-room caco = 3-HP caco). Each enemy carries `:type` for render lookup and optional `:max-concurrent` cap. Used by `build-world` for any vector-based `:enemies` field (e.g. L2-L9 mixes, and L10 boss arena where `{:type :cyber :count 1 :lives 50}` + `{:type :imp :count 2 :max-concurrent 1}` paints one boss + 2 minions with only 1 minion alive at once).
...
**Riser-gated chase (#301 part 1)**: each candidate step is also gated by floor height, mirroring the player's `physics/resolve-step`. A step onto a cell whose `:floor-pgrid` height is more than `physics/step-up-max` (0.4) above the enemy's current floor is rejected like a wall, so a monster can no longer 2D-walk UP a stair flank or tier riser taller than a knee-high step (no levitating between tiers). Descents always pass: an enemy on a ledge walks down off it toward the player. The coarse slide cascade still applies, so a chase that hits a tall flank slides along it and takes the open, climbable tread (the stair "mouth") instead of vaulting the side. Flat levels seed an all-0.0 `:floor-pgrid`, so the gate delta is always 0.0 < 0.4 and the chase stays wall-gated exactly as before (byte-identical).

**Stair-seeking (#301 part 2)**: when the coarse `chase-angle-offsets` cascade is fully blocked (every cardinal + diagonal step is a wall or a too-tall riser) AND a `:floor-pgrid` is present, `pick-step` falls through to a second pass, `seek-climbable-step`. It sweeps a finer angle fan (`stair-seek-offsets`: ±22.5° / ±67.5° / ±112.5° / ±157.5°, filling the gaps between the coarse rays), `try-step`s each (so every candidate is already 2D-open AND climbable), and steps onto the candidate whose floor height is closest to the *player's* floor (`|target-floor - dz|` minimised). So an enemy above the player picks the descending tread and one below picks the ascending tread - it mounts the stair mouth and routes toward the player's tier instead of freezing against the divider wall. This is greedy-with-detour, not a planner: it only finds a mouth that lies roughly along the current chase bearing (the four fixed L7 mouths are on-bearing when the player is up/down the same staircase). The second pass is `O(constant)` (8 `try-step` probes) and runs only on the frames an enemy is actually riser-blocked, so it adds nothing to the common chase frame. On flat ground (`:floor-pgrid` nil or all-0.0) the coarse pass always finds an open cell, so this pass is unreachable and the chase stays byte-identical.

**Bounded BFS-to-mouth (#311)**: the greedy stair-seek above only finds an *on-bearing* mouth, so an enemy that is OFF-BEARING from the only way up/down (a stair it is not roughly facing) descends one tier then wedges against the divider on the wrong tier - cross-tier combat stalls. The fix: when the enemy stands on a DIFFERENT floor tier than the player (`cur-floor != target-floor` in `step-toward`), the chase heading is no longer aimed straight at the player but at a cached bounded-BFS waypoint. `bfs-first-step` runs a breadth-first search over the grid from the enemy cell toward the player cell, expanding only cells that are not `wall?` AND climb-gated by `bfs-step-ok?` (the floor change to a neighbour is within ±`step-up-max` = 0.4 BOTH ways - so the route threads the smooth 0.375 stair treads and never plans a > step-up-max fall off a ledge), and returns the FIRST cell on the shortest reachable path; the enemy steers toward that cell's centre. The actual move still goes through `pick-step` / `try-step` / `land-step`, so the Z-gate, wall-slide, handrails and fall damage all still apply - BFS only chooses the heading. This routes the enemy to and through ANY stair mouth from ANY position (the L7 east-tier enemy walks west to stair D, down to tier 1, west to stair A, down to tier 0, to the player).

  Bounded + amortized so it stays off the hot path: the search stops as soon as the player cell is dequeued and caps at `bfs-max-visited` (1200, above L7's ~909 walkable cells) expanded cells; and the result is cached on the enemy (`:path-wp` next-cell, `:path-cd` countdown, `:path-ptier` the player floor the path was planned for) and recomputed (`refresh-path`) only every `path-recompute-frames` (12 ≈ 1.2s), on reaching the waypoint cell, or on a player tier change. Greedy still owns every SAME-tier frame: on a flat level (or once the enemy reaches the player's tier) `cur-floor == target-floor`, the cross-tier guard is false, BFS never runs, the path cache is cleared, and the chase is byte-identical to before. Step-phase cost (no-JIT local PHP, 7-enemy L7 roster, ~10x inflated so compare deltas): same-tier chase +0.3% (noise - BFS inert); the adversarial all-7-enemies-cross-tier-and-off-bearing worst case is +2.3ms/frame (~0.23ms with JIT), still well inside the step budget and far rarer than the same-tier common case.
...
**Feet anchoring (#297)**: A sprite is anchored by its FEET on the projected floor, not centred on the horizon. The renderer stands each billboard on `sprite-feet-row scene-pd svh dist eye-z fz pr` and draws the body UPWARD by its pixel height `h`, so the body spans rows `[feet-row - h, feet-row]` and the feet land exactly where the floor texture meets the wall at that distance. `fz` is the enemy's cell floor height, read from the world's `:floor-pgrid` (#232) at `(floor y, floor x)`; the raise is folded into `project-height` itself, so a monster on a raised cell (L5 dais `fz 0.8`, L7 ledge `fz 1.05`) projects its feet higher and stands ON the platform without any separate offset term. The grounding shadow, the face glyph, and the floating HP digit all derive from the same `feet-row` so they track the feet on flat AND raised ground. The earlier horizon-centred anchor (#234) left the feet hanging one row above the projected floor at most distances - the "enemies float" bug. Combat's vertical hit gate reuses the SAME `sprite-feet-row` so a shot lands on the drawn body, not the floor below or sky above it (#243 / #291 / #297). See `sprite-feet-row` in `core/projection` and `docs/raycaster.md` ("Sprite anchoring").
