...
**Dark platforms, orange stairs (#304 -> #322 art direction).** A multi-tier room (L7) first drew every raised platform, riser and cap in the same grey stone, so the structure was invisible. The #304 fix tinted whole tiers by height (cool steel-blue / magenta-rose), but those saturated hues read as floating coloured slabs ("use a darker color and not that pink blue shit"). The final scheme SPLITS a raised tier by ROLE: the flat PLATFORM surfaces (tier floors + horizontal step caps) are DARK near-neutral stone, while the vertical stair RISER FACES (+ nose edges) are ORANGE, so the stairs read as an unmistakable climbable ramp against quiet dark rock.

`palette/floor-tier` still buckets `fz` into a height band: `fz<=0` is tier 0 (ground), `0<fz<0.85` is tier 1, `fz>=0.85` is tier 2. The platform tables `shade-table-tier1` / `shade-table-tier2` darken the 24-level grey ramp (`tint-shade-table` with all-channel-<1 multipliers `0.80/0.83/0.90` and `0.62/0.64/0.72`, re-quantized through `nearest-256`) with only the faintest cool nudge, and the higher tier is darker than the lower, so the two plateaus step down in brightness as you climb. `tier-shade-tables` indexes them for the CAP path; `tier-bg-cell-caches` is the matching darkened twin of `bg-cell-cache` for the textured cap, so even the floor-cast cap reads as dark stone. The riser FACE + nose instead index `orange-shade-table` - the same 24-level fog ramp pushed hard toward orange (`1.55/0.78/0.18`), so a near step is bright amber and a far one a deep burnt orange (the stairs still fog with distance while staying orange). In `compute-wall-shades` the riser block now binds `cap-tbl` (dark tier stone) and `face-tbl` (orange) separately: `step-cap` reads `cap-tbl`, `step-face` + `step-nose` read `face-tbl` (the second riser mirrors it). Blood i-frames override BOTH with the red wash (the "you got hit" cue). The whole scheme is a strict no-op at `fz=0` (tier 0 is the literal grey `shade-table`), so every flat level - and the golden `render-cache-test/test-frame-bytes-pinned` fixture - is byte-identical (the 4 hashes unchanged). Hot-path cost is unchanged from #304: the riser/cap branch is still per-column (not per-cell) and the tables are baked once at load.
