...
```
aim-col =  reticle-axis(pointer_x, cols, sensitivity)
aim-row =  reticle-axis(pointer_y, rows, sensitivity)
...
- **`reticle-axis(p, len, sens)`** maps a 1-based pointer cell `p` to the reticle's 0-based viewport cell: `clamp(round(center + (p - 1 - center) * sens), 0, len - 1)` with `center = (len - 1) / 2`. The result is **rounded to the nearest whole cell** and returned as an int - the reticle is a discrete terminal cell, so one integer column keeps the painted `+`, the scene highlight and the hitscan angle on the same cell (a fractional value drifts them apart and trips PHP's float-to-int deprecation in the render `intdiv`). At sensitivity 1.0 it is **1:1** (a 20-cell pointer move is a 20-cell reticle move, and `raw` is already integral so the round is a no-op); a higher sensitivity moves the reticle further per pointer cell. The **clamp** is the key: when the OS pointer sits at or past the window edge, the reticle **pins** at the first / last cell instead of being lost. Returns nil for a nil / `<= 1` axis.
...
- `:pos` threads forward as the next frame's cached pointer (in `game-loop`, alongside `prev-keys`). `game-loop` seeds it to the terminal **centre** at game start and resets it to centre on a **resize** (a centred pointer places the reticle dead centre, so nothing auto-pans before a real report or when the absolute coords change meaning under a new size).

`commands/play` stamps the reticle cell on the world (`state/set-reticle`) while the mouse is on and resets it to centre (`state/reset-reticle`, the nil sentinel) when the mouse is off or on a resize; `apply-mouse-look` folds the edge-pan yaw/pitch into the player before the tick (so the frame renders + aims with the new heading), gated on not-paused so the camera can't drift while a menu is up. The world also carries `:scene-cols` (the cast width) so the off-centre hitscan can map the reticle column to a ray (see [Off-centre firing](#off-centre-firing-the-reticle-column-aims-the-shot)).

The reticle math (`reticle-axis`, `edge-pan-rate`) stays **pure** in `glue/controls` and is unit-tested (1:1 tracking, edge clamp / pin, centre-region vs band, ease-in shape, sign, sensitivity scaling on both reticle + pan, held-at-edge, return-to-centre stop, nil/degenerate axis). The escape re-assert that re-arms reporting lives in `io/` (next section).
...
A `Sensitivity` percent setting (0..100, step 10, default 50) maps through `core/settings.mouse-sensitivity` to a multiplier, **geometric** around the slider midpoint (issue #275): `3 ^ ((pct - 50) / 50)`. So 50% is the neutral 1.0x (kept exact for saved-settings back-compat), 0% slows to ~0.33x (1/3) for a gentle turn, and 100% speeds up to 3.0x for a fast spin. Each end sits the same ratio away from neutral (1/3x vs 3x). `mouse-aim` applies this multiplier to both halves of the reticle model: it scales the reticle's offset from centre (how far the crosshair travels per unit of pointer motion - exactly 1:1 at the neutral 1.0x) and the edge-pan turn rate (`steer-rate`'s output). At the default (50% / 1.0x) the far-edge yaw is `mouse-yaw-rate-max` 0.16 rad/frame (a full 360 in ~0.65s held at the edge); at 100% it is 0.16 * 3.0 = 0.48 rad/frame for a quick spin, and at 0% it is ~0.053 rad/frame for slow, deliberate turning. Sensitivity also scales how quickly the rate ramps in from the centre deadzone.
