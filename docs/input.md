...
### Positional steering (issue #318)

A terminal **cannot lock or warp the pointer** - there is no portable way to recenter the OS cursor, and the OS pointer cannot be hidden on most terminals (iTerm2 included, #313). A 1:1 **delta** model loses the pointer the instant a fast flick carries it off the window: the terminal then stops reporting, the camera freezes, and the OS pointer races on. So `mouse-look` (pure) drives the camera by **position, not delta**.

`controls/mouse-look` parses every report in the drained byte string, **caches the latest pointer position** (`:pos`, carried forward when no report arrives), and returns `{:yaw :pitch :fire? :fire-held? :pos}`. The camera turn-**rate** is a function of how far the cached pointer sits from the terminal **center**, applied **every frame**:

```
yaw   = steer-rate(pointer_x, cols, mouse-yaw-rate-max)   * sensitivity
pitch = -steer-rate(pointer_y, rows, mouse-pitch-rate-max) * sensitivity
```

- **`steer-rate(p, len, max-rate)`** for a 1-based pointer cell `p` on an axis `len` cells long: a central **deadzone** (`mouse-deadzone-frac` = 10% of the half-axis) produces **no turn** for precise aim, then an **ease-in** ramp (the fraction past the deadzone, squared - gentle near center) reaches `max-rate` at the far edge cell, signed by which side of center the pointer is on. It returns 0 in the deadzone, when `len` is nil, or when `len <= 1`.
- **Rate constants** (per frame at sensitivity 1.0): `mouse-yaw-rate-max` 0.16 rad/frame at the far left/right edge (a full 360 in ~0.65s held at the edge), `mouse-pitch-rate-max` 0.06 pitch-fraction/frame at the top/bottom edge. Pitch is the clamped [-1, 1] fraction via `state/clamp-pitch`, so holding the pointer above center ramps the view fully up (then saturates).
- **Pointer x right of center -> +yaw** (turn right), **left -> -yaw**. **Pointer y above center -> +pitch** (look up): rows grow DOWNWARD, so a pointer above center has a *smaller* y, hence the rate is **negated**.
- **Held off-center keeps turning.** The terminal sends reports only on **motion**, so a pointer held still off-center yields **no new bytes**. The steering reads the **cached** `:pos` every frame, so an empty `keys` string still returns the steering yaw/pitch from the last known position - the camera keeps spinning a full 360 while the pointer rests at an edge.
- **Escape-proof by design.** Pointer near center = no turn (aim finely); a small nudge toward an edge = turn, hold there = keep turning; return to center = stop. The pointer barely has to move, so it stays in the window. If it *does* leave at an edge, the last cached position keeps turning that way until the player brings it back - **recoverable**, unlike the old delta freeze.
- `dims` is the live `[rows cols]` (the `& [dims]` arg); it supplies the axis lengths `steer-rate` needs. Omitting it (used by any non-loop caller) makes the steering inert (zero yaw/pitch) while fire intent still parses, so nothing auto-spins without knowing where the edges are.
- `:pos` threads forward as the next frame's cached pointer (in `game-loop`, alongside `prev-keys`). `game-loop` seeds it to the terminal **center** at game start and resets it to center on a **resize** (a centered pointer steers zero, so the camera never auto-spins before a real report or when the absolute coords change meaning under a new size).

`commands/play.apply-mouse-look` folds the steering yaw/pitch into the player before the tick (so the frame renders + aims with the new heading), gated on not-paused so the camera can't drift while a menu is up.

The steering math (`steer-rate`) stays **pure** in `glue/controls` and is unit-tested (deadzone, edge saturation, ease-in shape, sign, sensitivity scaling, held-off-center, return-to-center stop, nil/degenerate axis). The escape re-assert that re-arms reporting lives in `io/` (next section).
