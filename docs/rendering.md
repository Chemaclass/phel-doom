...
**Feet anchoring (#297).** The renderer stands each billboard on its FEET, not centred on the horizon. `project-enemy` reads the enemy's cell floor height `fz` from the world's `:floor-pgrid` (#232) and carries it on the projection (`:fz`). The enemy zone pass computes `feet-row = sprite-feet-row scene-pd svh dist eye-z fz pr` (the projected floor surface row for that cell) and sets `e-top = feet-row - h`, so the body spans `[e-top, feet-row]` and the head/body/legs sub-zones cascade off `e-top` together. The grounding shadow drops on `feet-row`; the face glyph (`feet-row - h + h/6`) and the floating HP digit (`feet-row - h - 1`) ride the same anchor. A raised cell (L5 dais `fz 0.8`, L7 ledge `fz 1.05`) lifts `feet-row` because `project-height` at `z = fz` already folds in the riser - there is NO separate floor-offset term (adding one would double-count, the v1 bug). `scene-pd` is halved in pixel-doubled mode, so the projection scales there automatically without a manual `intdiv`. Flat ground (`fz = 0`) projects the feet onto the z=0 floor surface; the golden hashes in `render-cache-test/test-frame-bytes-pinned` are unchanged because the golden fixture has no enemy on-screen. The earlier horizon-centred anchor (#234, `(svh - h)/2 + pr - floor-off`) left the feet a row above the projected floor - the "enemies float" bug. See `sprite-feet-row` in `core/projection` and `docs/raycaster.md` ("Sprite anchoring"); the older `floor-offset-rows` projection field is retained but no longer drives the anchor.

**Cross-tier occlusion (#302).** Feet anchoring sets WHERE a cross-tier enemy stands; this clips it against the tier geometry IN FRONT of it, not just the full-height wall. Inside the zone pass, after the wall gate (`d < dists[c]`) each covered column intersects the billboard span `[e-top, e-bot)` with the column's nearer tier bands via the pure `clip-sprite-span` (`core/projection`):

```
clipped = (clip-sprite-span e-top e-bot
                            step-dists[c] shades-step-from[c]   ; nearer riser?
                            ceil-dists[c] shades-ceil-bots[c]   ; nearer ceiling?
                            d)
```

A riser NEARER than the sprite (`step-dists[c] < d`) clamps the feet to the riser band's on-screen TOP (`min(e-bot, shades-step-from[c])`); a nearer ceiling drop (`ceil-dists[c] < d`) clamps the head to the drop's bottom lip (`max(e-top, shades-ceil-bots[c])`). When the clamps cross, the helper returns `nil` and the column is skipped (fully occluded). The sprite path keeps the FULL box in `e-stop/e-sbot` (those drive the half-block emitter's sprite-y sampling) and writes the clipped `[c-top, c-bot]` into the separate visible-window buffers `e-vtop/e-vbot`; the emitter draws only `[e-vtop, e-vbot)` while sampling against `[e-stop, e-sbot)`, so a tier-clipped sprite is CROPPED at its true pixels rather than squished into the window. The glyph path writes the clipped span into `tops/bots` while keeping `mids/lowers` at the ORIGINAL `e-top`-anchored split (head/body/legs is anatomical, so a clip just stops the paint loop early without shifting the zones). `edists` is skipped when a column is fully occluded, so a hidden enemy neither occludes a pickup behind it nor drops a grounding shadow. The bands come from `compute-wall-shades` (destructured in scope at the gate); the tier distances from the cast. A FLAT level leaves `step-dists`/`ceil-dists` nil in every column, so both guards are false and the span is returned unchanged - the golden hashes stay pinned. Scope (px1, single nearest tier): no multi-tier stacking, px2 keeps the wall-only gate, and the shadow/face/pickup overlays still gate on the wall `dists` alone (cosmetic bleed, fast-follow). See `docs/raycaster.md` ("Cross-tier occlusion") for the geometry.
...
**Stair legibility: lit cap, shadowed face, nose line (#319).** The #232 riser drew the step cap and the vertical riser face within two shade levels of each other, with the cap the DARKER of the two, so a near step read as one flat tinted slab rather than a 3D step. The face/cap shading is now lit like a real staircase under overhead light: the horizontal CAP (tread top, the plane you look down on) brightens `+2` shade levels, the vertical FACE (riser front) drops `-3` into shadow, and the single top row of the face - where the lit cap rolls into the shadowed front - is painted a near-black NOSE line (`-8`, `step-nose`, set in `compute-wall-shades` and emitted at `row == step-tops` in the px1 cell loop). That cap-bright / face-dark contrast plus the dark nose edge is what reads the structure as discrete ascending steps. It rides the existing tier tint table (`tier-shade-tables`), so the nose/face/cap all stay tier-hued. A flat (`fz=0`) column never enters the riser clause (`step-from = vh`), so flat frames are byte-identical (`render-cache-test/test-frame-bytes-pinned`, hashes unchanged); px2 carries nil step data so it is unaffected. The first riser draws one legible step; the second (#322, below) draws the next tread up so a run reads as ascending steps.

**Second riser band (#322).** The cast now emits the next riser up the staircase (`:step2-*`, see [raycaster.md](raycaster.md#second-riser-322)). `compute-wall-shades` projects it into a second band (`step2-from/bots/tops/face/cap/nose`) exactly like the first riser, with one extra clamp: its lower edge is capped at the FIRST riser's top row (`step-tops`), so the near step occludes the base of the far step and the two read as an ascending pair rather than overlapping slabs. The px1 cell loop paints it in a clause right after the first riser (the second band sits at higher rows, above the first, so they never fight for a row). The second cap uses the flat tier shade (`step2-cap`) rather than the textured `floor-cap-px` of the first - it is farther and smaller, so the cheap flat cap reads fine and keeps the hot cell loop lean - while the second nose (`step2-nose`, at `row == step2-tops`) and face match the first riser's lit-cap / dark-face / nose lighting. nil `:step2-*` (flat world, single-step run, or px2) means `step2-from = vh` so the clause is dead, and flat frames stay byte-identical (the golden hashes hold). Measured cost: the cast change is ~+10% cast-phase on the L7 stair scene and the cell-loop clause brings the full frame to ~+5-6% there; both are ZERO on flat levels.

**Sprite ground-platform (#319).** An enemy standing on a raised tier was projected with its feet correctly LIFTED (`sprite-feet-row`, #297) but with no tier floor surface drawn under it - the renderer only paints the single nearest riser and a flat `z=0` floor - so a monster on a platform behind a divider wall or up a stair read as FLOATING in mid-air. After the grounding-shadow pass, a third pass paints a short PLATFORM PAD under any enemy on a raised cell (`fz>0`): a bright tier-tinted lit lip on the feet row (`tier-platform-nose`) over one to three darker tier-tinted face rows below (`tier-platform-face`, depth scaling with apparent size), one column wider than the sprite each side, so the billboard unmistakably stands ON tier stone. Both cells are BG-only space cells (not `▀` glyphs) so a run of identical pad cells RLE-coalesces; legibility comes from the bright-nose / dark-face brightness step. It writes the top-priority `blood-paint` overlay and reuses the shadow pass's `enemy-front-visible-at?` wall/enemy occlusion gate, and unlike the grounding shadow it runs in BOTH sprite and glyph modes (the float reads worst with the default billboards). Drawn ONLY when `fz>0`, so a flat-ground enemy (every flat level) paints no pad and flat frames stay byte-identical.

**Per-tier floor tint (#304).** A multi-tier room (L7) drew every raised platform, riser and cap in the same grey stone, so the stepped structure was invisible. Each raised tier now reads in a distinct hue keyed on the cell's floor height. `palette/floor-tier` buckets `fz` into a band: `fz<=0` is tier 0 (ground), `0<fz<0.85` is tier 1, `fz>=0.85` is tier 2 - the 0.85 split lands the L7 middle tier (0.75) and the L5 dais (0.8) cool, and the L7 top tier (1.5) plus its 1.125 stair rose. Tier 1 is a cool steel-blue tint, tier 2 a magenta-rose tint. Rose is chosen over a warm amber on purpose: the stone palette is already warm-grey, so an amber tier would blend in, while rose is absent from the grey/brown/orange band and reads distinct at any distance; cool-blue + rose also form a complementary cold/warm pair so the two tiers never compete in the same hue half. The tints are baked at load: `tint-shade-table` hue-shifts each of the 24 grey shade levels by per-channel RGB multipliers and re-quantizes through `nearest-256`, producing `shade-table-tier1` / `shade-table-tier2` (parallel to `shade-table`); `tier-shade-tables` indexes them by tier so the riser-face + flat-cap path is a single branch-free table swap, and `tier-bg-cell-caches` is the matching tinted twin of `bg-cell-cache` for the textured cap. Luminance is preserved, so the distance-fog ramp and EW side-shading still read; the deepest fog levels quantize back to neutral, so a FAR tier fades into haze rather than glowing. The tint is a strict no-op at `fz=0` (tier 0 is the literal grey `shade-table`), so every flat level - and the golden `render-cache-test/test-frame-bytes-pinned` fixture (build-world 1, all `fz=0`) - is byte-identical (the 4 hashes are unchanged). Blood i-frames still override the tint (the red wash wins for the "you got hit" cue). Hot-path cost is one extra `aget` on the riser/cap branch (already a per-column, not per-cell, path); the lookup tables are baked once at load. The multipliers were TONED DOWN (#322): the first pass pushed blue to 1.45x / red to 0.55x, which `nearest-256` snapped to vivid cube colours so a raised tier read as a solid NEON-BLUE band ("the tiers look horrible"). The toned multipliers (cool `0.70/0.84/1.30`, rose `1.30/0.64/1.14`) keep the tier clearly distinct from grey and from each other while reading as cooled / warmed STONE rather than a colour flood; the bulk of the tier legibility now comes from the lit-cap/dark-face/nose lighting and the two-step (#322) structure, with the hue as a supporting cue.
...
## Hanging ceilings (#235)

The mirror of the raised-floor riser on the ceiling axis. The cast surfaces an additive `:ceil-*` side-channel (the first ceiling a column crosses that DROPS below the viewer's, nil per column otherwise); `compute-wall-shades` + the px1 cell loop turn each drop into a vertical **hanging face** plus a flat-shaded **cap** (the ceiling underside), hanging from the top of the view above the main wall and claimed before the sky-cast clause so the sky does not also paint it. A height ABOVE 1.0 just lifts the ceiling (extra headroom, no hanging edge, so nothing is drawn). A flat (all-1.0) ceiling leaves every `:ceil-*` entry nil, so the branch is dead and the frame is byte-identical (pinned by `render-cache-test/test-frame-bytes-pinned`); no current level ships a `:ceil-heights` map yet. Cast-side detail and the field table: [raycaster.md](raycaster.md#per-cell-ceiling-heights-235).
