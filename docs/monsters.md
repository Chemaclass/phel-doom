...
**Feet anchoring**: A sprite is anchored by its feet on the enemy's own cell floor row (`round(svh/2 - (enemy-fz - eye-z)*wall-px) + pr`, issue #375), not centred on the horizon; on flat ground (`enemy-fz` 0, `eye-z` 0.5) this reduces to `svh/2 + 0.5*wall-px`. The renderer stands each billboard on that row and draws the body upward by its pixel height, so a monster on a raised tier stands on the platform rather than sinking to z=0. The grounding shadow, the face glyph, and the floating HP digit all derive from the same foot row. Combat's vertical hit gate (`enemy/vertical-hit?`, via `aim-pr` / `sprite-half-rows`) anchors on the SAME cell floor height (issue #396), so a shot lands on the drawn body even when the enemy is on a raised tier: at level aim an elevated enemy sits above the fixed screen-centre crosshair (missed until you aim up), and aiming up drops the sprite onto the crosshair - see == shoot. On flat-ceiling shipped levels the gate is byte-identical to before (`enemy-fz` 0, `eye-z` 0.5). `combat` threads `(:fz-grid world)` + `eye-z` (`aim-eye-z`) into `shoot` / `pierce` / `spread-shoot`; legacy shorter arities pass a nil grid + 0.5 eye and stay flat.
