...
- Per-cell ceiling-height data layer (issue #385, epic #375 Phase 3): worlds now carry `:cz-grid` (per-cell ceiling heights, default 1.0 = full height) with a flat PHP hot-array twin `:pcz` (indexed `y * width + x`) and a build-time `:cz-flat?` flag, mirroring #368's floor `:fz-grid` / `:pfz` / `:fz-flat?`. `map/ceiling-z` reads a height (1.0 out of bounds / nil grid); layouts author lowered ceilings via lowercase chars `a`/`b`/`c` (cz 0.75/0.5/0.25). A cell with raised `fz` and lowered `cz` is a window gap. Saves round-trip `:cz-grid` (`:pcz` rebuilt on load, defaults to full-height for pre-#385 saves) without a version bump. Data only - cast/render/physics still assume ceiling z=1 and every shipped level stays full-height (zero behavior change); the cast/render/physics-combat sub-issues (#386-#388) make ceilings visible + walk-blocking.
- Distinct riser shading + per-tier tint (issue #373, epic #375): riser faces (the vertical rise of a step) previously shaded exactly like a coplanar wall, so a step read as an ordinary wall slice. `riser-shade-level` now pulls the riser index darker than the wall it derives from, and nudges it darker again by tier height (`z1` in fz-step units, 1/2/3), so a step never looks like a same-distance wall AND taller rises read progressively distinct - elevation is legible at a glance. Lands entirely in `compute-riser-shades` (once per column, not per cell), so flat levels are byte-identical (every non-tiered golden hash unchanged) and tiered render cost is unchanged (bench within noise at 80/120/180 cols). Tier-top floor texturing (the other half of the polish pass) is deferred to a focused follow-up: `tier-top-row-dist` stays wired-and-ready, and the flat-gradient tier top remains visually adequate; a single-per-column texel approximation risked reading worse than the current smooth gradient without a perf-gated visual pass.
- Staircase showcase level (issue #372, epic #375): L2 is now a verticality showcase. Its `showcase-layout` gains a 3-tread staircase (fz 0.25/0.5/0.75 via layout digits `1`/`2`/`3`) climbing north from the spawn to a raised 0.75 platform, viewable from below at the spawn and from above on the platform; the L2 entry drops `:walls` so `scatter-walls` can't chew the treads. New golden-hash frames pin the shipped layout rendered from below, from above, and pixel-doubled; a new physics test walks the player up the stairs and asserts `:z` climbs one `fz-step` per tread (0 -> 0.25 -> 0.5 -> 0.75) then holds on the platform, with a companion test that a two-step rise halts the climb. Enemies still spawn on the ground tier and render at world z=0 (platform-enemy feet-lift is deferred to a follow-up, same as #370's sprite anchoring).
