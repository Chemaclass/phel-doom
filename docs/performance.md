...
- **Cadence**: a uniform ~120fps target. The 30fps cap was artificial - on hardware fast enough to render a wide-terminal frame inside the 8.33ms budget the player was pinned to 30fps for no reason, and on hardware too slow to make it the per-iteration sleep already floors at `min-yield-us`. Framerate now tracks render capability up to the 120fps ceiling and degrades smoothly toward render-native below it. Every per-frame input feel constant (movement/turn/pitch/sprint hold timers, mouse edge-pan rate) is stored in seconds / per-second units, not a fixed frame count, so this cap can move without changing input feel.
